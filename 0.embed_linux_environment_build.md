...
1. VMvare 15.5.1 
2. Ubuntu16.04 LTS/Ubuntu22.04 LTS

选择>新建虚拟机>典型, 选择安装文件*.iso，直接下一步安装，等待完成即可.<br/>
**注意:因为wsl1是Microsoft模拟实现的linux子系统，所以缺少很多linux相关组件，这在交叉编译时往往卡很久，所以做嵌入式开发一定不要用wsl1环境，wsl2环境还要确定使用Hype-V虚拟机系统环境(需要开启Hype-v服务)，否则会遇到很多莫名其妙的异常问题,一定要注意.**
...
&emsp;&emsp;[linux平台软件安装.md](./0-1.linux平台软件安装.md) <br />

## linux平台软件编译

&emsp;&emsp;对于linux平台，除了软件源提供的可以直接获取的软件包，另一部分就是需要手动安装或者编译安装的软件包，这里列出总结的软件编译方法，详细见下午。<br />
&emsp;&emsp;[linux平台软件编译.md](./0-2.linux平台软件编译.md) <br />
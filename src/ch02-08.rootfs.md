...
[busybox构建最小Linux文件系统](#busybox_create)
[buildroot构建Linux文件系统](#buildroot_create)
[Debain构建Linux文件系统](#debain_create)
[基于Ubuntu Core的文件系统](#ubuntu_create)
...
BR2_TOOLCHAIN_EXTERNAL_PATH="/usr/application/coding_git/remote_manage/sdk/support/arm/compiler"
...
#因为我使用的编译工具不支持rpc，所以需要二次修改，取消rpc功能
#这一步不能带配置，否则会自动修改为y
make menuconfig
Toolchain>Toolchain has RPC support?>not selected
...
#3.修改镜像源和dns服务器，地址https://mirrors.tuna.tsinghua.edu.cn/help/ubuntu-ports/
...
#4. 支持/sbin/init入口
apt-get install sudo vim systemd libterm-readkey-perl -y
...
exit
while grep -Eq "${workdir}.*(dev|proc|sys)" /proc/mounts
do
...
直接开始下一章节说明: [ch02-09 qemu虚拟机环境构建](./ch02-09.qemu.md)

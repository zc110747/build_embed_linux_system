...
在Linux系统中，输入设备数量繁多，从单个引脚的输入按键，按键矩阵，到复杂的触摸屏，鼠标，键盘。这些设备接口总类繁多(如gpio，i2c，usb)，器件特征也有很大差异。引脚输入为单个I/O电平信号，按键矩阵则可能是一组电平信号，触摸屏和鼠标输入的则是位置信号，键盘输入的又是对应键位的扫描码，可以说五花八门，各有不同。内核为了统一管理这些设备，系统实现input子系统，在系统中就可以通过/dev/input/eventX设备，按照固定的格式访问解析。常见的input子系统设备按照输入数据类型的不同可分成如下两类。
...
- [按键input驱动实现方法](#input_key_driver)
- [内核key-gpios使用方法](#input_key_kernel)
- [触摸屏input驱动实现方法](#input_ts_driver)
- [iput应用层解析](#input_app)
...
至此，可以看到判断key作为input设备的驱动实现完成，详细代码可以见[key-input驱动代码](./file/ch03-05.input_subsystem/kernel_input.c)。
...
相关代码参考：[input-key应用实现代码](./file/ch03-05.input_subsystem/test/key_input_test.c)。

...
在Linux系统中，输入设备数量繁多；常见的有输入按键，按键矩阵，扩展I/O、触摸屏、鼠标、键盘等。这些设备接口总类多样，如gpio、i2c、usb等，器件特征也有很大差异。例如普通按键输入为I/O电平信号，按键矩阵则是一组电平信号，触摸屏和鼠标输入的则是位置信号，键盘输入的又是对应键位的扫描码；可以说各有特点，应用也大不不同。

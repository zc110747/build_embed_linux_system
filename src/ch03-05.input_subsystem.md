...
至此，可以看到判断key作为input设备的驱动实现完成；完整的input-key驱动代码如下所示。

- [key-input驱动代码](./file/ch03-05/kernel_input.c)
...
- [input-key应用实现代码](./file/ch03-05/test/key_input_test.c)
...
上述功能实现了input-ts作为input设备的驱动完整完成；详细代码如下。

- [key-ts驱动代码](./file/ch03-05/kernel_gt9147.c)
...
本文分别以按键和触摸屏得例子描述了如何通过input子系统实现驱动，并通过应用层获取按键和触摸屏事件，阐述了基于input子系统得驱动和应用开发的流程。不过在实例中，往往需要结合具体的硬件，实现不同的组合功能；这部分就需要不断积累总结提高，在实践中可以参考内核中drivers/input/目录下驱动实现和相关的Documentation目录下的文档。以这篇文章为基础，结合应用实践，才能不断进步提高。

本文涉及的相关代码如下所示。

- [input-key驱动代码](./file/ch03-05/kernel_input.c)
- [input-key应用访问代码](./file/ch03-05/test/key_input_test.c)
- [input-touchscreen驱动代码](./file/ch03-05/kernel_gt9147.c)

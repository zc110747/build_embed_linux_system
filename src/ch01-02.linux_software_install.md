...
Linux根据不同平台支持不同的安装命令，如debian系的apt-get，centos的yum，本系列因为使用ubuntu系统属于debian，因此都使用apt/apt-get作为安装命令，对于打包好的文件安装包，可以使用dkpg进行安装；这里列出与安装的相关命令。
...
- [7zip解压缩工具](#7zip_install)
- [openssh工具安装](#openssh_install)
- [tftp服务安装](#tftp_install)
- [nfs服务安装](#nfs_install)
- [node环境安装](#node_install)
- [python安装和模块下载](#python_install)
- [vsftpd服务器安装](#vsftpd_install)
- [iperf测试吞吐量](#iperf_app)
- [shellcheck使用](#shellcheck_install)
- [docker服务](#docker_install)
...
3. 输入远程登陆方法，"ssh freedom@192.168.3.29", 选择connect
...
server tftp
{
    ...
    server_args = -s /home/freedom/Desktop/remote_manage/sdk/build/tftp_root/arm -c
    per_source = 11
    cps =100 2
    flags =IPv4
}
...
TFTP_USERNAME="tftp"
#此处指定tftp的映射目录
TFTP_DIRECTORY="/home/freedom/Desktop/remote_manage/sdk/build/tftp_root/arm"
TFTP_ADDRESS=":69"
TFTP_OPTIONS="--secure"
```
...
/home/freedom/Desktop/remote_manage/sdk/build/nfs_root/arm *(rw,sync,no_root_squash)
```
...
```shell
setenv bootargs "console=ttymxc0,115200 root=/dev/nfs nfsroot=192.168.3.29:/home/freedom/Desktop/remote_manage/sdk/build/nfs_root/arm,proto=tcp rw ip=192.168.3.99:192.168.3.29:192.168.3.1:255.255.255.0::eth0:off"
saveenv 
reset
```

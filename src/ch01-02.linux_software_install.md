...
Linux根据平台和环境的不同，支持不同的安装命令。如debian系的apt-get，centos的yum，本系列因为使用ubuntu系统属于debian，因此都使用apt/apt-get作为安装命令(当然Ubuntu后面推出了snap，不过个人习惯使用apt命令)。

当然，对于文件安装包，使用dkpg/rpm进行安装；这里列出与安装的相关命令。
...
- [Linux平台软件安装使用方法](#linux平台软件安装使用方法)
  - [综述](#综述)
    - [install\_command](#install_command)
  - [software\_install](#software_install)
    - [vscode\_install](#vscode_install)
    - [7zip\_install](#7zip_install)
    - [openssh\_install](#openssh_install)
    - [tftp\_install](#tftp_install)
    - [nfs\_install](#nfs_install)
    - [node\_install](#node_install)
    - [python\_install](#python_install)
    - [vsftpd\_install](#vsftpd_install)
  - [iperf\_app](#iperf_app)
  - [shellcheck\_install](#shellcheck_install)
  - [docker\_install](#docker_install)
  - [apache2\_install](#apache2_install)
...
vscode可以通过setting.json管理项目文件的显示，再根目录创建.vscode目录，其下创建setting.json文件，内容如下。
...
server tftp
{
    ...
    server_args = -s /home/freedom/Desktop/user_git/remote_manage/sdk/tftp -c
    per_source = 11
    cps =100 2
    flags =IPv4
}
...
TFTP_USERNAME="tftp"
#此处指定tftp的映射目录
TFTP_DIRECTORY="/home/freedom/Desktop/remote_manage/sdk/tftp"
TFTP_ADDRESS=":69"
TFTP_OPTIONS="--secure"
```
...
#本地测试tftp工作是否正常
touch /home/program/build/tftp_root/test.c
...
/home/freedom/Desktop/remote_manage/sdk/build/nfs *(rw,nohide,sync,no_root_squash)
```
...
直接开始下一小节: [Linux shell脚本语法](./ch01-03.linux_shell_grammar.md)。

...
- 如果script指定加载脚本文件存在(boot.scr)，则加载此文件并执行(在imx6ull中其实是通过loadscript加载)
...
- mmcdev指定系统对应的硬件接口, 通过**mmc list**可以查看系统支持的硬件，以NXP imx6ull开发板为例， 0表示SDCard, 1表示mmc设备.
...
bootcmd保存了一系列U-Boot执行的启动命令，这些命令在 U-Boot 启动倒计时结束后会被执行, bootcmd可以通过CONFIG_BOOTCOMMAND定义在xxx_defconfig中，包含一系列在U-Boot环境下可以执行的命令(boot.src内容类似)，这里以imx6ull-mmc的启动命令进行解析.

...
- [Debian编译构建脚本](#build_sh)
- [chroot沙箱定制Debian系统](#chroot_structure)
  - [进入沙箱模式](#enter_chroot)
  - [定制化构建Debian系统](#customized_sys)
...
```shell
# 安装必要的软件
apt-get install vim net-tools dialog apt-utils sudo -y
```
...
```shell
I: Checking Release signature E: Release signed by unknown key (key id F8D2585B8783D481) The specified keyring /usr/share/keyrings/debian-archive-keyring.gpg may be incorrect or out of date. You can find the latest Debian release key at https://ftp-master.debian.org/keys.html
```
...
gpg --dearmor | sudo tee /usr/share/keyrings/debian-archive-keyring.gpg > /dev/null
```

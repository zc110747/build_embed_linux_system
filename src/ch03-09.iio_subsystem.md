...
Linux iio(industrial I/O)是用来统一处理各种传感器和ADC/DAC模块的框架。在没有使用IIO驱动框架之前，对于传感器和ADC/DAC模块，如果进行资源访问，需要创建对应的字符设备，实现读写接口，然后通过应用层进行访问。

传统字符设备访问方式当然可以，不过对于传感器、ADC、DAC等模块资源来说，应用层访问就比较分散。在传感器种类越来越多的情况下，应用层访问就显得冗杂，不利于应用层的项目维护。IIO模块则提供了一个通用的接口，使得不同类型的传感器可以方便地集成到Linux系统中，通过统一管理，来满足高效使用的需求。

对于IIO子系统，其功能和目录实现如下所示。

📑 **IIO框架说明**  
📑 **设备文件注册接口和配置**  
📑 **基于IIO框架的模块驱动实现(内部adc模块)**  
📑 **基于IIO框架的模块驱动实现(HX711驱动)**  
📑 **在其它驱动中调用IIO设备**  
...
可以看到，IIO子系统和字符设备驱动接口都是提供应用层访问硬件的接口，对于底层操作硬件的实现是相同的，如通过i2c，spi，以及内部总线，访问硬件寄存器。

不过IIO子系统框架可以将类似功能的传感器，ADC/DAC模块通过"/sys/bus/iio/devices/iio:deviceN"目录进行访问，统一管理。另外IIO子系统也定义了一套标准的设备驱动接口，使得传感器和转换器的驱动程序可以基于这些接口进行开发，支持数据的采集、转换和处理，包括模拟信号到数字信号的转换、数据的滤波和校准等功能，这样大大简化了应用层的开发和使用，也是其优势所在。

基于字符设备的开发流程，IIO子系统也类似，只是将设备文件的创建流程替换为IIO子系统的构建流程，具体如下。

🧱 *设备树中添加硬件节点，驱动程序中匹配执行初始化程序*  
🧱 *实现对硬件的初始化，以及访问接口，包含读取，写入，控制等接口*  
🧱 *关联IIO访问驱动接口和硬件访问接口*  
🧱 *配置IIO注册的结构体，在内核中IIO设备*  

## 设备文件注册接口和配置

对于IIO模块，支持的主要函数如下所示。

📘 `devm_iio_device_alloc`：用于分配一个新的IIO设备结构体  
📘 `devm_iio_device_register`：IIO 设备注册到内核中。注册后，设备将可以被用户空间应用程序访问和控制  
...
📘 `info`：用于访问设备的接口信息，包括读取，写入，权限控制等接口  
📘 `channels`：用于描述设备的通道信息，包括通道类型，通道编号，通道地址等信息  
📘 `num_channels`：用于描述设备的通道数量  
📘 `iio_buffer_setup_ops`: 用于设置设备的缓冲区操作接口  
...
到这一步，对于iio设备驱动部分的实现大致解析完毕，详细代码如下所示。

📄 iio adc驱动代码：`file/ch03-09/iio_adc/kernel_iio_adc.c`

### 基于内部ADC的应用层访问接口

对于iio设备的访问，主要就是对于`/sys/bus/iio/devices/iio:deviceX/`目录下的设备文件操作。
...
关于hx711模块的驱动代码实现完毕，详细驱动代码如下所示。

📄 驱动代码文件：`file/ch03-09/iio_hx711/kernel_iio_hx711.c`
...
📄 adc_joystick文件：`drivers/input/joystick/adc_joystick.c`
...
至此，关于iio子系统框架的代码功能和实现讲解完毕。

IIO子系统为开发者提供了一个统一的框架来实现和管理传感器和数据采集设备，通过合理使用IIO子系统提供的接口和结构体，可以方便地完成设备驱动的开发和应用层的访问。

...
WDT又被称为看门狗，是用于监视系统运行的模块。其主要功能用于当系统跑飞或者死机时, 无法进行喂狗时, 产生某些动作重启设备。目前主流的看门狗有内部的系统看门狗，以及外部的硬件看门狗。

虽然功能和配置可能不同，但其控制原理和配置都类似，包含如下参数信息。
...
struct watchdog_ops {
    ...
    int (*start)(struct watchdog_device *);                                 // 看门狗启动时调用函数(核心函数)
    /* optional operations */
    int (*stop)(struct watchdog_device *);                                  // 看门狗结束时调用函数(核心函数)
    int (*ping)(struct watchdog_device *);                                  // 看门狗发送持久连接ping函数，用于复位(核心函数)
    unsigned int (*status)(struct watchdog_device *);                       // 看门狗显示设备状态函数
    int (*set_timeout)(struct watchdog_device *, unsigned int);             // 看门狗设置超时时间函数(核心函数)
...
上述代码为访问内部的wdt设备，详细代码如下所示。

- WDT驱动代码：`file/ch03-15/kernel_wdt.c`
...
- WDT应用层访问代码：`file/ch03-15/test/wdt_test.c`

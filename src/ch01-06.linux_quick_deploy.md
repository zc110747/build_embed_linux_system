...
4. 支持rootfs编译或构建方法(busybox，buildroot(nogui/qt)，debain，ubuntu，yocto，android......)，对于单个项目基本选择一种即可。
...
rootfs是用于管理和存储的一系列的文件的集合，这部分可以自己编译(如busybox，buildroot)，或者使用已构建好的系统，并在此基础上扩展安装(如debain，ubuntu)，此部分参考[rootfs环境构建](./ch02-08.rootfs_overview.md)中的相关说明，实现构建系统的构建。
...
至此，关于如何构建一个SDK的流程进行了大致说明，虽然实现逻辑并不复杂，不过其原理上包含构建嵌入式Linux工程的所有方面，没有一定的基础，理解起来有一定的困难。获取系统运行的u-boot，kernel，rootfs，添加应用需要的至此库，再将应用导入，就可以实现最终的产品，而部署就是将这些流程通过shell命令组合的方式，进行指令化，这就大大提高了效率。

Linux SDK的构建需要对于Linux基础，编译系统，shell脚本，平台构建的综合知识，是需要坚持不懈的积累，当然本篇也只是构建SDK的一种思路，不同芯片往往也有不同的方案，不过从原理上是相通的，理解了SDK的构建方法，也就能够一法通万法，很快掌握其它SDK的使用方法。

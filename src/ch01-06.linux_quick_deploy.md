...
对于Linux开发的项目，第一步最重要的是安装SDK文件包，然后在基础上进行修改，编译，开发的工作，这是大部分嵌入式开发的工作流程。SDK是整个项目开发的基础，一般也十分复杂，除了构建SDK的人，即使后续维护的人也很难去理解清楚各个方面。我参与过大型医疗设备的后续维护开发，虽然产品功能开发完成，但是也还花费了小半年时间才真正的理解了框架，后续在自己的项目实践后，才算真正理解SDK的构建和部署方法，这里进行详细说明。

## structure

在理解SDK的构建方法之前，我们先说明下完整嵌入式Linux项目需要的组成部分。

1. 交叉编译工具(gcc)，不过不同组件可能需要不同版本的编译工具
2. U-Boot源码，配置文件、代码，编译脚本
3. Kernel源码，配置文件、代码，编译脚本
4. 支持rootfs编译或构建方法(busybox，buildroot(nogui/qt)，debain，ubuntu，yocto，android......)
5. 适配硬件的设备树和驱动程序(gpio，i2c，spi，rtc，pwm......)
6. 应用运行的支持库和可执行文件(openssl，mosquitto......)
7. 应用软件和编译构建脚本
...
rootfs是用于管理和存储的一系列的文件的集合，这部分可以自己编译(如busybox，buildroot)，或者使用已构建好的系统，并在此基础上扩展安装(如debain，ubuntu)，此部分参考[rootfs环境构建](./ch02-08.rootfs.md)中的相关说明，实现构建系统的构建。
...
最后剩余的就是应用代码，以及SDK构建和编译的脚本，这部分参考第四部分的说明。另外，构建能够快速编译上述各部分的命令需要脚本的支持，这就涉及linux命令以及shell语法的说明，可参考[shell命令说明](./ch01-01.linux_shell_cmd.md)和[shell语法说明](./ch01-03.linux_shell_grammar.md)。到这里，可以看出来，构建一个完整的项目SDK，就是理解和应用嵌入式Linux系统的过程，是对各部分技术和支持的运用，这在下一部分进行说明。

## deploy

SDK再原理上其实和SDK类似，是由目录，源码，编译脚本共同组成的，能够编译出u-boot，zImage，dts，rootfs，support library，application的集合，理解了这一点，就可以规划sdk的目录和编译环境。
...
可以看到SDK中包含基础系统的构建部分，再此基础上，我们添加应用实现具体的功能，其中make.sh就是系统构建的脚本，具体如下。
...
上述就是编译sdk的脚本，基于此，可以使用简单的命令，如"./make.sh uboot"即可完成uboot的编译，并将最终生成的u-boot-dtb.imx放置再package目录下，可直接下载使用。当然，对于完整的系统，还有进一步的操作，将命令组合再通过export导入到系统中，这样使用全局命令即可编译，具体实现如下。
...
至此，关于如何构建一个SDK的流程进行了大致说明，其实现逻辑并不复杂，获取系统运行的u-boot，kernel，rootfs，添加应用需要的至此库，再将应用导入，就可以实现最终的产品，而部署就是将这些流程通过shell命令组合的方式，进行指令化，这就大大提高了效率，不过这需要对于Linux基础知识和系统的理解，这就要坚持不懈的积累，再此共勉。

...
本节从命令开始，讲解Linux平台应用层处理的脚本机制，具体目录如下所示。
...
shell语法支持依赖于支持的运行环境，常见的有`bash、ash、korn、tcsh、zsh`等。其中命令一般指的是在命令行模式下可以直接使用的可执行命令，主要包含内置命令，以及存在于系统目录(echo $PATH)中的`/bin/，/usr/bin/，/usr/sbin/`和`$PATH目录`指定下的可执行程序或者脚本文件。

对于Linux应用来说，脚本和程序按照来源不同如下所示。
...
这些命令的最重要特征是可以执行，对于全局的命令可以通过whereis或which指令来查找位置，部分指令可以通过`man [command]`或`[command] -h`查看说明(需要在安装时有相应的说明文件)。

本篇对于命令的说明只能说是沧海一粟，对于实际运用中，需要长期的经验积累不断学习。

这里列出总结时的参考文档和书籍，可以配合一起学习。

- 《Linux命令行和Shell脚本编程大全(第四版)》
- 《鸟哥的Linux私房菜 基础学习篇》

注意:**本章节中所有的指令只提供说明和例程指令，如果需要具体的功能和参数，建议在Linux系统中使用man命令查看，或者参考书籍系统的学习。**

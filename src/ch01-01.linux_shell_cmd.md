...
在学习Linux系统构建之前，还是需要了解下shell命令，这也是使用Linux平台的基础技能，在Linux平台的开发和应用都依赖这些命令。shell命令一般指的是在Linux命令行模式下可以直接使用的可执行命令，这些命令存在于目录"bin/", "/usr/bin/"以及系统$PATH目录指定下的可执行程序或者脚本，这些脚本和程序按照来源不同一般分为如下脚本。

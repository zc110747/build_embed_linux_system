...
SPI是一种高速，高效率的串行接口技术，通常由一个主模块和一个或多个从模块构成；此接口主要用于芯片和芯片，芯片和器件之间的数据传输通讯。

SPI设计上为主从结构，在工作时通过主机提供时钟信号，发送数据的同时从接收引脚上读取数据，同时实现数据接收和发送功能，因此为全双工模式。

SPI设计上没有类似I2C的地址来区分从机设备，对于多从机的通讯，主要依赖CS引脚来选中芯片工作。当SPI总线上有多个芯片使能，写入不影响，但多从机读取时会发生总线竞争，有可能导致接收数据异常。当管理多个器件时，设计上需要每个芯片拥有独立的片选控制CS引脚。

SPI接口主要由CS、CLK、MOSI和MISO组成，详细说明如下所示。
...
SPI外部设备是挂载在SPI总线的设备，由SPI总线，SPI器件设备树实现，SPI器件驱动三部分组成。

关于SPI模块的说明，具体目录如下所示。

- 总线模型和内核加载
- 设备树实现和解析
- 外部器件驱动应用(以icm20608为例)
- 总结说明
...
外部器件驱动主要实现基于spi总线提供的内核接口，管理应用层和外部器件的接口。
...
对于spi_sync/spi_sync_transfer/spi_write_then_read接口，返回0并不一定表示外设已经正确接收，只是表示内部控制器完成了传输。因此需要通过读取ID、读取配置或者结合波形来判断释放正确写入数据。
...
- 器件icm20608驱动实现：`file/ch03-08/kernel_spi_icm.c`
...
- 应用读取示例代码：`file/ch03-08/test/icm_spi_test.c`

## 总结说明

至此，关于SPI总线驱动，以及外部器件的驱动初步讲解完成；和I2C类似，SPI并不是特别复杂的接口，不过连接的器件类型总类同样繁多，类似FLash、OLED、3-axis传感器、ADC芯片、库仑计芯片、网络芯片W5500等，都是使用SPI接口。

SPI框架作为访问硬件的通道，将驱动和物理器件的访问关联起来，实现通讯处理。不过对于硬件如何添加到内核，实现应用层的访问方法，就需要根据器件所属的功能模块，具体问题具体分析，才能实现完整的功能。

...
使用BusyBox构建系统的方法，可以看到通过复杂的源码编译、目录创建、库移植和文件打包的流程；最后生成的还是仅支持系统命令的最小Linux文件系统。在具体的应用中，可能还需要图形(QT)，网络(mqtt、http)，数据库(qslite、mysql)，容器服务(docker)；这就需要第三方库支持，也就是需要大量的移植工作，可以参考如下文档，处理繁杂的兼容性问题。

- [嵌入式Linux平台软件的交叉编译](./ch01-04.linux_cross_compiler.md)

如何避免这些复杂的工作，将基础系统和第三方库以源码的形式整合；然后使用自动化脚本，配置文件的方式进行管理系统功能，实现自动化构建；Buildroot就是基于这个思路实现的。为了详细说明Buildroot，本节从源码结构、编译和安装、编译选项等多方面进行说明。

关于Buildroot的具体目录如下所示。
...
Buildroot是以BusyBox、Systemd等为核心，集成编译工具、第三方库、可执行文件和系统服务脚本的，用于文件系统的自动化构建方案。对于目前最新的Buildroot，是支持Uboot和Kernel编译构建的；不过个人不习惯使用，Uboot和Kernel下载和编译时间很长，而且Buildroot支持的版本并非较新的版本，还需要单独移植支持板级设备。因此使用外部的U-Boot和Kerenl自行移植(参考前面的章节实现)。另外Buildroot支持版本一般是主线的分支，不一定支持用户的开发板，需要单独的移植。本文中Buildroot作为文件系统进行单独编译，这里先讲解下目录中跟编译和构建相关的功能。

- arch 存放CPU架构相关的配置脚本，编译U-Boot和Kernel需要。
- board 定义板级设备信息，说明开发板使用的配置项和配置文件，可集成到最终打包项目中。
...
buildroot添加自定义配置，可使用buildroot的overlay功能，主要说明如下。

```shell
# 1. buildroot创建目录，保持覆盖的配置文件
mkdir -p borad/arm/imx6ull/rootfs-overlay

# 2. 在rootfs-overlay下根据buildroot的目录格式添加文件，如etc/network/interfaces

# 3. 编译选项选择overlay目录，这里使用defconfig修改(也可在菜单中System Configure下设置)
# overlay support
BR2_ROOTFS_OVERLAY="board/arm/imx6ull/rootfs-overlay"
```

之后重新编译buildroot即可，不需要每次编译后重新修改。

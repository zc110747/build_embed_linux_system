...
- 如果选择文件系统为busybox，使用make menuconfig进行buildroot的配置界面，使用make busybox-menuconfig可以进入busybox的配置界面
- buildroot的菜单可以控制软件是否安装，不过具体的版本在package下定义，以ace为例，如果希望更改版本，则需要修改ace.mk和ace.hash两个实现，当然如果希望增加不存在的软件到buildroot中，也可以在package目录下添加。

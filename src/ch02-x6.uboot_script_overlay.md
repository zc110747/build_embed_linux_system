...
上述脚本是定义在源码中，如果希望修改执行脚本，只能重新编译整个U-Boot并重新下载才能够实现。

U-Boot自然提供一种更通用的模式加载脚本，这就U-Boot支持的加载和source命令。

这里给出两种典型的例子，分别从网络和emmc中加载脚本，并执行。

```shell
# 通过tftp从网络加载脚本文件，然后执行脚本
tftp <load_addr> <file>; soruce;

# 以fatfs格式从mmc中读取脚本文件，然后执行
...
上述步骤就是通过命令将脚本文件从其它地方(Emmc，Nand，Tftp)加载到指定内存地址，使用source命令加载写入的内存地址即可执行,这就给开发者在U-Boot实现脚本提供了方法。

source命令处理的脚本文件，需要是序列化转换成二进制的文件，这里需要通过mkimage命令进行打包转换。
...
前面说了，source命令可以从指定内存区域加载**序列化的脚本**命令进行执行。

这里序列化的脚本，就是将txt文件转换成的二进制脚本文件。二进制脚本文件可通过mkimage(U-Boot目录中生成打包可执行文件的工具)编译脚本文件实现，关于mkimage的命令说明如下。
...
基于boot.scr脚本，启动流程修改为如下所示。

1. U-Boot启动查找bootcmd变量，执行加载命令。
2. bootcmd命令从不同地址加载boot.scr脚本。
3. boot.scr脚本执行编译成二进制的脚本。

基本上所有的U-Boot系统都支持source命令，这基本上是U-Boot的通用功能。

关于U-Boot的脚本内容，可以参考此处实现：[boot.cmd脚本](./file/ch02-x6/boot.cmd)。

## config

使用U-Boot的脚本，开发者可以使用setenv，tftp等命令加载boot.scr，从而控制后续执行流。不需要进行复杂的代码修改，以及编译下载，只要替换生成的boot.scr文件就可以实现目的。不过这种方法仍然是有缺陷的，加载的是序列化打包的二进制脚本，也就是说每次修改脚本都需要执行mkimage重新打包，然后替换打包后脚本文件才能生效。

对于已经运行的系统，是能够看到此Boot.scr脚本文件，不过修改起来仍然比较麻烦，对于大部分嵌入式Linux环境也不支持mkimage命令，往往需要跨平台进行打包处理。这里就需要一种更直接修改配置文件的方法。

利用env命令，可以直接加载文本格式的配置文件，不需要转换，这样就更简单的实现功能。
...
基于此方法，可以在config.txt文件中使用[key]=value的形式配置系统参数。系统启动后，直接将目录挂载到文件系统中，修改txt文件即可直接替换U-Boot的环境变量。

关于config.txt的实现可以参考[config.txt配置文件](./file/ch02-x6/config.txt)。

## dtbo_overlay

设备树插件DTBO（Device Tree Blob Overlay）是一种用于Linux内核和嵌入式系统的重要机制，主要用于动态地修改或扩展系统运行时的设备树配置。对于设备树插件，主要依赖于U-Boot的脚本和fdt命令，其流程如下所示。

1. 执行U-Boot脚本功能，在指定地址加载设备树*.dtb文件和内核
2. 重置设备树区域，使用fdt命令在设备树区域中加载*.dtbo文件并合并
3. 使用修改后的设备树，传递给内核，完成系统启动

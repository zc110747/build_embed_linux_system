...
对于u-boot命令行应该都不陌生，在系统启动中使用按键打断即可进入uboot命令行模式，使用printenv，setenv，bootz等命令来设置启动参数，控制跳转等。在之前的章节[U-Boot环境变量和命令管理](./ch02-04.uboot_env.md)中进行了详细的说明。不过在u-boot中执行脚本，可能很多人并不熟悉，其实在u-boot中执行脚本并不复杂，主要依赖如下命令。
...
也就是说先通过命令将脚本从其它地方(mmc，tftp)加载到指定内存地址，使用source命令加载写入的内存地址即可执行，这就给开发者在u-boot实现脚本提供了方法，具体步骤参考后续说明，目录如下所示。
...
上面使用u-boot的脚本，开发者可以使用setenv，tftp等命令控制设备硬件和执行流。不用进行复杂的重新编译下载，已经实现了目的。不过这种方法仍然是有缺陷的，加载的是序列化打包的二进制脚本，也就是说每次修改脚本都需要执行mkimage重新打包替换后才能生效。对于已经运行的系统，即使能够看到此脚本文件，修改起来也比较麻烦。这里就需要一种更直接修改配置文件的方法，这个功能是存在的，利用env命令，可以直接加载文本格式的配置文件，不需要转换。
...
uboot执行加载基于fdt命令实现，主要包含三个关键命令，如下所示。

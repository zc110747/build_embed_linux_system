...
- [gpio操作接口](#gpio_api)
  ...
  - [gpio_free](#gpio_free)
  - [of_get_name_gpio](#of_get_name_gpio)
  - [of_get_name_gpio_flags](#of_get_name_gpio_flags)
...
- [i2c总线接口](#i2c_bus_api)
  ...
  - [i2c_master_recv](#i2c_master_recv)
  - [i2c_master_send](#i2c_master_send)
  - [i2c_type](#i2c_type)
...
- [fb子系统](#fb_api)
  ...
  - [fb_deferred_io_cleanup](#fb_deferred_io_cleanup)
...
gfp: 申请数据的类型
    ...
    GFP_ATOMIC:在中断、软中断、spinlock等原子上下文里面，申请内存，应该使用GFP_ATOMIC标记，申请尽可能不触发内存回收，避免休眠
...
- 功能: 用于管理platform总线驱动移除和加载的宏，同时定义platform_driver_register和platform_driver_unregister功能
- 目录: include/linux/platform_device.h
...
- 功能: 获取platform总线匹配设备中interrupts属性对应的irq编号
- 目录: include/linux/platform_device.h
        drivers/base/platform.c
...
- 功能: 获取platform总线匹配设备中interrupts属性中的中断个数
- 目录: include/linux/platform_device.h
        drivers/base/platform.c
...
index: 获取资源的编号, 如果对于一个资源包含多个，按照编号访问

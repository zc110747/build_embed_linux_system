...
Linux背光管理框架用于控制和管理设备的背光亮度。该框架提供了统一的接口，允许不同类型的背光设备（如 LCD 屏幕、键盘背光等）通过标准的方式进行亮度调节。
...
另外则都是操作PWM的代码，这部分可以参考PWM章节中关于consumer的说明。

关于背光驱动代码详细如下。

- backlight驱动：`file/ch03-13/kernel_pwm_bl.c`
...
- backlight应用：`file/ch03-13/test/brightness_test.c`

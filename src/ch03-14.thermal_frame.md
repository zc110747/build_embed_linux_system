...
其中thermal_governor从thermal_zone_device获取区域温度，然后根据当前温度，决定调用哪个降温设备来为该区域降温。可以看到thermal的处理即涉及温度采样这类ADC相关的操作，也涉及风扇控制这类PWM相关的操作，因此在理解本节前，建议对前面的[iio子系统驱动框架和ADC模块驱动](./ch03-09.iio_subsystem.md)和[PWM子系统设备框架](./ch03-12.pwm_frame.md)熟练的掌握，本节中会利用相关的知识。
...
2. thermal_zone 设备抽象与注册：基于温度采样设备硬件，抽象成thermal_zone设备，注册到thermal core框架中
3. 降温设备驱动实现，可能是PWM/DAC控制的风扇，或者CPU主频、电压调节器等
4. thermal_cooling 设备抽象与注册：基于降温设备硬件，抽象成thermal_cooling设备，注册到thermal core框架中
...
温度采样设备的驱动主要是ADC，CPU内部温度寄存器或I2C/SPI接口连接的温度sensor；注册到内核后以iio provider的形式可以被其它驱动访问，这部分可以参考:[ch03-09.iio子系统驱动框架和ADC模块驱动](./ch03-09.iio_subsystem.md)相关文档，这里从thermal_zone设备的实现开始说明。
...
关于thermal_zone的详细代码见: [thermal_cooling驱动文件](./file/ch03-14/thermal_cooling/thermal_fan.c)。
...
thermal_frame是比较复杂的设备，可以看到涉及iio-adc采样，thermal_zone温控采集管理，thermal_cooling温控输出，pwm风扇控制输出，因此这部分一般由Linux集成的驱动实现，很少由用户自己实现，不过理解了整个管理流程，就可以更简单的去理解如何修改设备树，去调整来期望设备工作在指定温度。

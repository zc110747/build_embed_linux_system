...
温度采样设备的驱动主要是ADC，CPU内部温度寄存器或I2C/SPI接口连接的温度sensor；注册到内核后以iio provider的形式可以被其它驱动访问，这部分可以参考:[ch03-09.iio子系统驱动框架和ADC模块驱动](./ch03-09.iio子系统驱动框架和ADC模块驱动.md)相关文档，这里从thermal_zone设备的实现开始说明。

...
thermal_frame是比较复杂的设备，可以看到涉及iio-adc采样，thermal_zone温控采集管理，thermal_cooling温控输出，pwm风扇控制输出；这部分一般由Linux集成的驱动实现，很少由用户自己实现。不过理解了整个操作流程，就可以更直观的去学会如何修改设备树，通过调整阈值来期望设备工作在指定温度。

...
嵌入式Linux系统使用I/O来说与外界交互，简单的如gpio，i2c, spi, 复杂的如cmos，lcd，eth，usb接口，都由I/O来作为信号的输出或者输入。因为功能的复杂性，以及引脚的数目限制，每个引脚都可以复用成不同的功能，这就需要一套机制来处理引脚的复用机制。pinctrl子系统就是在这个基础上引入的。通过在设备树中配置引脚的复用，在内核中解析设备树并处理，即可直接完成引脚的复用功能和输入输出性能的配置，可以说简化了驱动的开发，也避免了直接对物理寄存器的操作，是目前主流的引脚复用管理方式，另外gpio作为最基础的应用，因为耦合度较高，这里也同pinctrl一起讲解说明。
...
因为pinctrl主要用于解析设备树，并导入到内核中，因此主要修改的部分在设备树中，在设备树中以iomux和iomuxc_snvs节点显示，
以I.MX6ull中支持两种pinctrl类型的应用为例，其设备树如下。
...
struct pinctrl {
  ...
  struct device *dev;         -- 使用pinctrl资源的设备节点
...
从内核设备树中获取gpio资源的接口有两类，包含gpio和gpiod接口，从功能上来说，gpio和gpiod都能实现配置引脚输入输出状态，并控制输入和获取引脚状态的功能，不过gpio属于老的接口，gpiod则是引入的新的API，支持通过GPIO_ACTIVE来控制逻辑电平和实际电平，关于gpio的接口说明如下。
...
在实际开发中，建议优先使用gpiod相关接口进行i/o管理，这里列出相关接口，对于接口的详细说明见ch03-x1和ch03-x3章节。
...
1. "export" - 用户空间可以通过写其编号到这个文件，要求内核导出一个 GPIO 的控制到用户空间。例如: 如果内核代码没有申请 GPIO #19,"echo 19 > export"，将会为 GPIO #19 创建一个 "gpio19" 节点。
...
1. "direction" ... 读取得到 "in" 或 "out"。这个值通常运行写入。写入"out" 时,其引脚的默认输出为低电平。为了确保无故障运行，"low" 或 "high" 的电平值应该写入 GPIO 的配置，作为初始输出值。注意:如果内核不支持改变 GPIO 的方向，或者在导出时内核代码没有明确允许用户空间可以重新配置 GPIO 方向，那么这个属性将不存在。
2. "value" ... 读取得到 0 (低电平) 或 1 (高电平)。如果 GPIO 配置为输出,这个值允许写操作。任何非零值都以高电平看待。
...
4. "active_low" ... 读取得到 0 (假) 或 1 (真)。写入任何非零值可以翻转这个属性的(读写)值。已存在或之后通过"edge"属性设置了"rising"和 "falling" 沿触发模式的轮询操作(poll(2))将会遵循这个设置。

...
pinctrl子系统就是在这个基础上引入的，通过在设备树中配置引脚的复用，在内核中解析设备树并处理，即可直接完成引脚的复用功能和输入输出性能的配置。pinctrl简化了驱动的开发，也避免了在代码中直接对寄存器的操作，降低了开发难度，是目前主流的引脚管理方式。当引脚复用为普通gpio时，也定义了一系列接口进行管理，这部分和pinctrl耦合度较高，因此这里一起讲解说明。关于gpio和pinctrl框架如下所示。

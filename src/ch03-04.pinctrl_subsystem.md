...
嵌入式Linux系统使用I/O与外界器件交互，简单的如gpio，i2c, spi, 复杂的如cmos，lcd，eth，usb接口，都由I/O来作为信号的输出或者输入。因为功能的复杂性，以及引脚的数目限制，每个引脚都可以复用成不同的功能，这就需要一套机制来处理引脚的复用机制。
...
从内核设备树中获取gpio资源的接口有两类，包含gpio和gpiod接口，从功能上来说，gpio和gpiod都能实现配置引脚输入输出状态，并控制输入和获取引脚状态的功能，不过gpio属于老的接口，**gpiod则是引入的新的API，支持设备树中通过GPIO_ACTIVE来控制逻辑电平和实际电平(gpio接口使用gpiod_set/get_raw_value，不受该配置影响)**，关于gpio的接口说明如下。

...
嵌入式Linux系统使用I/O与外界器件交互，简单的如GPIO，SPI，I2C，复杂的如CMOS，LCD-RGB，ETH，USB接口等，都是由I/O引脚来作为信号的输出或者输入接口。因为功能的复杂性，以及引脚的数目限制，每个引脚都可以复用成不同的功能，这就需要一套机制来处理引脚的复用功能，这就是pinctrl框架。另外当引脚作为普通I/O时，又需要在驱动中控制引脚的电平机制，gpio子系统正是用来实现对引脚的控制。

pinctrl框架主要通过在设备树中配置引脚的复用，内核中加载解析，即可直接完成引脚的复用功能和驱动能力的配置。gpio子系统则通过封装的接口，管理引脚资源，控制引脚状态，进行对寄存器操作的封装。这样就简化了驱动的开发，也避免了在代码中直接对寄存器的操作，降低了开发难度，是目前主流的引脚管理方式。
...
//获取I/O设备的管理控制信息，并同时设置pinctrl到名称指定配置
...
1. gpio是内部模块，没有具体总线，因此需要支持platform管理，只能定义在根节点，或者simple-bus等特殊节点(设备树章节有说明)
2. gpio作为设备节点，需要有两个最基本的属性，"compatible"和"status"，分别用于驱动匹配和设备状态管理
3. gpio作为引脚功能，需要pinctrl定义引脚复用为I/O, 需要pinctrl-0节点，对应pinctrl-names则可以省略，如果只支持单个复用，值要为default字符串，表示默认复用
4. gpio需要控制输入输出功能，需要指定[name]-gpios属性主要用于在驱动中使用devm_gpiod_get查询到指定gpio，控制输出电平，不过of_get_named_gpio这些老接口支持全名访问gpio资源，没有限制，不过这里还是建议所有gpio资源都使用"[name]-gpios=<...>"的格式

参考这个思路，最基础的LED的设备树的定义如下。
...
至于驱动的实现以及应用层的访问，和上章节说明基本一致，这里不重复说明，详细参考: [LED驱动文件](./file/ch03-04.pinctrl_device/kernel_led.c)，不过其中引入了复用功能切换，sysfs文件管理的相关内容。

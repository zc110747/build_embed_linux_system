...
static int pcf8563_get_time(struct device *dev, struct rtc_time *tm)
{
    ...
    ret = pcf8563_read_block(client, PCF8563_REG_SECONDS, buf, 7);
    if (ret) {
...
static int pcf8563_set_time(struct device *dev, struct rtc_time *tm)
{
    ...
    ret =  pcf8563_write_block(client, PCF8563_REG_SECONDS, buf, 7);
    if (ret) {
        dev_err(&client->dev, "pcf8563_write_block failed:%d\n", ret);
        return ret;  
    }
    return 0;
}
...
static int pcf8563_clear_alarm_flag(struct i2c_client *client)
{
    unsigned char buf;
    int ret;
    ...
    return pcf8563_write_block(client, PCF8563_REG_CONTROL2, &buf, 1);
}
...
static irqreturn_t pcf8563_irq_handler(int irq, void *data)
{
    ...
    dev_info(&client->dev, "alarm:%d!\n", pending);

    pcf8563_clear_alarm_flag(client);
...
// 匹配结构体
// .of_match_table匹配设备节点中的compatible
// .pm用于电源管理
// .id_table用于匹配老版本的设备资源表

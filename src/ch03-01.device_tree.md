...
对于嵌入式Linux平台，一般由内核(如Cortex-A，RISC-V)，然后通过系统总线(AHB、APB、AXI等)连接GPIO、I2C、SPI、PWM、CAN、ETH等外设模块组成。对于具体的产品，又由SOC搭配各种外部器件，如EPPROM、SENSOR、PHY(ETH、CAN)、CMOS、FLASH等外部器件实现不同的功能。对于早期内核版本，为了处理SOC和外部硬件结构的差异，在设备代码中维护一套硬件表格来管理芯片和板级的差异。

这样虽然能够实现功能，不过不同芯片、不同板级的硬件驱动就全部集成在驱动中，包含大量的冗余代码；另外对于板级硬件的改动，即使只是一点，都需要驱动代码修改和生成，如果驱动代码本身集成在内核中，还需要内核的整体编译。ARM芯片设计的碎片化又加剧了这一问题，这给代码的合并带来巨大的麻烦。为了解决这一问题，统一设备模型被设计处理哎。通过总线、设备和驱动分离的机制来管理这些复杂的硬件设备。对于客观存在的设备差异，则引入DeviceTree来抽象描述硬件信息。设备树在启动后被U-Boot传递到内核(通过bootcmd定义)，在内核加载时解析设备树创建对应的总线和设备。驱动工程师只要针对硬件修改设备树和实现对应的驱动即可适配不同的硬件，这就简化了开发方式。关于设备树的语法说明，可以参考如下文档。
...
对于网络上有经验分享或者设备树中已经支持的设备，会告诉你如何添加可以实现功能；可是为什么这样可以实现，却是复杂的问题。对于芯片原厂或外部器件厂商新研发的产品，可能市面上并没有经验分享，又如何通过设备树进行支持。这些疑惑在最初学习了解设备树时一直困扰着我，如果去关注内核dts目录下的所有设备树，会发现每家厂商设备树在框架上基本一致，但在具体的细节上却大相径庭。理解了NXP的设备树，再去看全志、瑞芯微的设备树，似乎还是有很大障碍。这是正常的，设备树在看似简单的语法下，其实维护一套复杂的描述体系；在内核的版本迭代支持，也在同步更新和迭代，学习并不简单。

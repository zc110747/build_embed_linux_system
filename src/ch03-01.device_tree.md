...
对于客观存在的设备差异，则引入DeviceTree来抽象描述硬件信息。设备树在启动后被Uboot传递到内核，在内核加载时解析创建对应的设备和总线。这样驱动工程师，只要针对硬件修改设备树和实现对应的驱动即可，简化了开发方式。关于设备树的语法说明，参考网址.

- [设备树说明文档](https://elinux.org/Device_Tree_Usage)

设备树是描述硬件的数据结构，是对硬件结构的抽象。每个硬件节点都映射到实际的器件，如RTC，I2C, SPI，在实际SOC或硬件PCB中都存在相应的模块和器件。这看起来很清晰，不过当进行软件开发时，例如在PCB中I2C引脚挂载带中断引脚输入的RTC，如何添加对应的设备树节点，一个带两路ADC和一路开关的摇杆，又如何在Linux系统中加载对应驱动？对于网络上有经验分享的设备，会告诉你如何添加即可实现功能，可是为什么这样就可以实现，却是复杂的问题。对于芯片原厂，一个器件如果是新研发的，网络上没有资料，那又如何添加到设备树。我也是带着这些疑惑去梳理设备树的总结，希望对理解这些问题有帮助。

如果去关注内核dts目录下的所有设备树，会发现每家厂商的实现在框架基本一致，在具体的细节上却差异。理解了NXP的设备树，再去看全志，瑞芯微的设备树，似乎还是有很大障碍，这也是我最初接触设备树的最大烦恼。设备树在语法的限制其实是严格的，"中括号的用法，节点的定义，引用节点的方法，冒号作为语句的结束，如何引入其它文件，节点的继承和覆盖"，这些基础的规则，违反了会直接解析出错。但是具体到节点中的属性，包含属性名和值的定义，又会觉得差异巨大。

在设备树中，属性名的定义分两类，内核定义的标准属性，以及厂商和用户的私有属性，它们的特性完全不一样。

1. 标准属性：在设备树中定义的，具有特定功能，与内核中相应接口配合访问的属性定义。这类属性的名称和类型都是固定的，有些还要符合一些规则。如compatible表示节点标识符，类型需要为一个或多个字符串类型，在驱动匹配时使用，也可以被of_device_compatible_match访问。这类属性并不少，在后面小节会说明。

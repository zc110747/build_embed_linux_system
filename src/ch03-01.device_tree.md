...
对于嵌入式Linux平台，一般由内核(Cortex-A、RISC-V等)，然后通过系统总线(AHB、APB、AXI等)连接GPIO、I2C、SPI、PWM、CAN、ETH、PCIE等外设模块组成。对于具体的产品，又由SOC搭配各种外部器件，如EPPROM、SENSOR、PHY(ETH、CAN、USB)、CMOS、FLASH等外部器件实现不同的功能。

为了满足复杂的硬件外设需求，Linux基于设备树机制来管理设备间的差异，在源码中可通过`arch/arm(arm64)/boot/dts`查看。
...
对于早期内核版本，为了处理SOC和外部硬件结构的差异，在设备代码中维护一套硬件表格来管理芯片和板级的差异。这样虽然能够实现功能，不过不同芯片、不同板级的硬件驱动就全部集成在驱动中，包含大量的冗余代码。

对于板级硬件的改动，即使只是稍微修改，都需要驱动代码修改和生成。另外驱动代码集成在内核中，还涉及内核的整体编译，修改起来十分复杂。而ARM芯片设计的碎片化涉及又加剧了这一问题，这给代码的更新迭代带来巨大的麻烦。为了解决这一问题，设备模型被设计出来；通过总线、设备和驱动分离的机制来管理这些复杂的硬件设备。对于客观存在的设备差异，则引入DeviceTree来抽象描述硬件信息。设备树在启动后被U-Boot传递到内核(通过bootcmd定义)，在内核加载时解析设备树创建对应的总线和设备。驱动工程师只要针对硬件修改设备树和实现对应的驱动即可适配不同的硬件，这就简化了开发方式。关于设备树的语法说明，可以参考如下文档。
...
这些就是最基础的规则，构建设备树的根基，违反了一般会直接解析出错，新手这一步就常常出错。

## 设备树中的属性说明

对于设备树节点中的属性，包含属性名和值的定义，就会差异巨大；这也是前面感觉割裂的原因。设备树语法严谨但功能却十分灵活，属性更是可以根据驱动需求进行扩展，学习和理解设备树就是一个很大的挑战。
...
上面讲到了通过设备树可以描述板级的设备硬件，那么Device Tree是否需要描述系统中的所有硬件信息呢？这个答案是否定的，能够被动态探测的设备是不需要设备树描述的，例如USB Deivce(如U盘，USB网卡，USB键盘等)；在系统启动时并不存在，而是在系统运行过程中，通过USB总线动态的添加到系统中。得益于设备模型，这类设备可以先将驱动挂载到总线上；动态探测设备时，在系统中就会创建相应的设备节点，并执行对应的程序即可实现全部功能。

...
在前面说明驱动模型的时候，定义了总线，设备和驱动来实现对硬件的操作。在硬件中，总线(如i2c, spi等)是SOC和外部器件的连接通道, 在其上挂载着各类器件。不过对于内部模块，如gpio、rtc、wdg、csi等设备，是通过AHB或AXI连接到CPU Core上，这并不属于需要配置的总线。不过为了驱动的统一管理，Linux系统中对于这些内部设备，定义了platform总线来管理。在设备树中，满足以下条件的设备节点会被platform总线管理。

...
如果去关注内核dts目录下的所有设备树，会发现每家厂商的实现在框架基本一致的情况下，内容却都有差异。理解了NXP的设备树，再去看全志，瑞芯微的设备树，似乎还是有很大障碍，这也是我接触设备树的最大迷惑。原因很简单，设备树语法在结构上虽然限制比较严格，例如中括号的用法，节点的定义，引用节点的方法，冒号作为语句的结束，如何引入其它文件，节点的继承和覆盖，这些都是统一的基础方法。但是具体到变量，包含变量名称和值的定义，却又显得并不严格。
...
内核专用变量因为和内核的访问有关，需要有严格的限制才能在内核启动后被解析，然后在驱动中使用函数访问，如果只是去理解语法，不去掌握背后的与内核的关联，那么就会迷惑为什么要这样定义。而自定义变量只要符合变量语法，即[key]=[value]；又毫无其它限制，那么理解这些变量，又必须去驱动中找寻相应的实现才能够知晓功能，只看语法意义就不大，很多时候需要参考芯片手册才能理解含义。

设备树学习的困难之处就在于此，任何一个文档都无法理清楚设备树中所有变量定义的功能需求和逻辑，需要配合着内核API接口，芯片寄存器说明以及驱动去理解。上面说了一大段话，不过你可能还是感觉很抽象，那么这里配合全志的t113_i中的twi，也就是i2c驱动去理解，这里同样进行了备注。

```c
twi2: twi@2502800{
    #address-cells = <1>;                           //子节点reg相关专用变量，表示reg中解析地址时个数
    #size-cells = <0>;                              //子节点reg相关专用变量，表示reg中解析地址长度时个数
    compatible = "allwinner,sun8i-twi";             //内核专用变量，标识符，用于内核匹配twi2模块，位于platform总线管理
    device_type = "twi2";                           //内核专用变量，设备类型
    reg = <0x0 0x02502800 0x0 0x400>;               //内核专用变量，twi2的寄存器地址
    interrupts = <GIC_SPI 11 IRQ_TYPE_LEVEL_HIGH>;  //内核专用变量，twi2中断控制器，中断线号和触发信号
    clocks = <&ccu CLK_BUS_I2C2>;                   //内核专用变量，twi2总线使能时钟
    resets = <&ccu RST_BUS_I2C2>;                   //内核专用变量, twi2总线复位时钟
    clock-names = "bus";                            //内核专用变量，twi2总线时钟别名，对应clocks
    clock-frequency = <100000>;                     //内核专用变量，twi2时钟频率
    status = "disabled";                            //内核专用变量，twi2模块状态
};

&twi2 {
    clock-frequency = <100000>;                     //内核专用变量，twi2时钟频率
    pinctrl-0 = <&twi2_pins_a>;                     //内核专用变量，引脚复用模式默认模式，i2c引脚
    pinctrl-1 = <&twi2_pins_b>;                     //内核专用变量，引脚复用模式休眠模式，gpio引脚
    pinctrl-names = "default", "sleep";             //内核专用变量，引脚复用别名
    twi_drv_used = <1>;                             //内核自定义变量，表示是否使用dma fifo模式，启动后dmas属性才被访问
    dmas = <&dma 45>, <&dma 45>;                    //内核专用变量，用于dma控制外设发送和接收
    dma-names = "tx", "rx";                         //内核专用变量。dma定义别名，驱动中访问channel通道
    status = "okay";                                //内核专用变量，twi2模块状态

    rtc@68 {
        compatible = "dallas,ds1307";               //内核专用变量，标识符，用于驱动匹配设备节点
        reg = <0x68>;                               //内核专用变量，rtc器件寄存器地址，驱动访问i2c节点使用
    };
};
...
在之前讲驱动模型的时候，提供了上述图来说明总线，设备和驱动的关系。可以看到，在硬件中，总线是SOC外设和外部器件的连接通道(如i2c, spi等), 在其上挂载着各类器件。不过对于内部模块，如gpio, rtc, i2c controller等设备，是通过内部总线连接到CPU Core上，并不属于需要配置的总线。为了统一管理，Linux系统中对于这些内部设备，定义了platform总线(可通过/sys/bus/platform查看)来管理。在设备树中，满足以下条件的设备节点会被platform总线管理。
...
基于以上信息，我们理解DTS/DTSI是基于DTS语法实现的设备描述文件，DTB则是编译后用于内核解析的二进制文件，也是下载时需要使用的文件。dts语法看起来所见即所得，是一种典型的描述语言，其实内部有很多约束和规定，看起来简单，但适配起来其实相当复杂。
...
对于设备树来说，基本单元就是node。node从根节点开始，被组织成树状结构，添加不同的设备节点描述的。以比较简单的按键节点为例:
...
resets: 模块复位线号，一般是复位时钟相关位
reset-names: 复位资源别名，驱动中访问

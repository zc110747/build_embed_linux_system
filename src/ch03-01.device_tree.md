...
=======
设备树是描述硬件的数据结构，是对硬件结构的抽象。每个硬件节点都映射到实际的器件，如RTC，I2C, SPI，在实际芯片和PCB中都存在相应的模块和器件。这看起来很清晰，不过当我们运用到开发时，例如在PCB中I2C引脚挂载带中断引脚输入的RTC，添加对应的设备树节点该如何实现？网上一定有相应的教程，告诉你如何添加即可实现功能；可是为什么这样就可以实现。对于芯片原厂，一个器件如果是新研发的，网络上没有资料，那又如何添加到设备树。是不是你也在疑惑，带着这个疑问，进行本篇的学习，会有不一样的理解。

设备树的实现为什么如此迷惑，而且每一家似乎都有差异。理解了NXP的设备树，去看全志，瑞芯微的设备树，似乎还是有很大障碍。其实原因很简单，设备树语法在结构上虽然限制比较严格，例如中括号的用法，节点的定义，如何引用节点，冒号作为语句的结束，节点的继承和覆盖，这些都是统一的。但是具体到变量名称和值，却又显得并不严格。在设备树中，变量名的定义分两类，内核定义的专用变量和用户自定义的变量，它们的特性完全不一样。

1. 设备树专用的变量: 在设备树中定义的，具有特定功能，与内核中相应接口配合访问的变量定义，这类变量的名称和类型都是固定的，有些还要符合一些规则。如compatible表示节点标识符，类型需要为一个或多个字符串类型，在驱动匹配时使用，也可以被of_device_compatible_match访问，如果节点需要被驱动匹配，就一定要有该属性。这类变量并不少，在后面小节会说明。
2. 由用户或者芯片厂商自定义的变量: 自定义变量没有特定的功能和类型的限制，在需要时定义，描述开发时需要约束或者配置的功能，在驱动中使用通用访问接口如of_property_read_u32进行读取用于后续配置和操作。对于厂商来说，自定义变量一般以公司名开始，当然也有直接定义变量，如nxp的"fsl,max_ddr_freq = <400000000>";，全志的"sunxi,audio-codec = <&codec>;",这类定义就需要在使用过程中去了解。

内核专用变量因为和内核接口函数有关，需要有严格的限制才能在内核启动后被解析，然后在驱动中使用接口访问，如果只是去理解语法，不去掌握背后的与内核的关联，那么就会迷惑为什么要这样实现。而自定义变量只要符合变量语法，即[key]=[value]；又毫无其它限制，那么理解这些变量，又必须去驱动中找寻相应的功能才能够知晓定义的原因，只看语法意义并不大。这就是设备树学习的困难之处，任何一个文档都无法理清楚设备树中所有变量定义的功能需求和逻辑，需要配合着内核接口，设备驱动去理解才能去掌握。不过这并不是说总结是无意义的，本文还是会整理一些内核的变量功能，抛砖引玉，可以说为深入学习指引方向。

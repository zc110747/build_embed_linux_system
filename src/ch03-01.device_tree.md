...
对于嵌入式Linux平台，一般由内核(Cortex-A、RISC-V等)，然后通过系统总线(AHB、APB、AXI等)连接GPIO、I2C、SPI、PWM、CAN、ETH、PCIE等外设模块组成。对于具体的产品，又由SOC搭配各种外部器件，如EPPROM、SENSOR、PHY(ETH、CAN、USB)、CMOS、FLASH等来共同实现不同的功能。

对于早期内核版本，为了处理SOC和外部硬件结构的差异，在设备代码中维护一套硬件表格来管理芯片和板级的差异。这样虽然能够实现功能，不过不同芯片、不同板级的硬件驱动就全部集成在驱动中，包含大量的冗余代码；对于板级硬件的改动，即使只是一点，都需要驱动代码修改和生成。另外驱动代码集成在内核中，还涉及内核的整体编译，修改起来十分复杂。ARM芯片设计的碎片化涉及又加剧了这一问题，这给代码的更新迭代带来巨大的麻烦。

为了解决这一问题，统一设备模型被设计出来。通过总线、设备和驱动分离的机制来管理这些复杂的硬件设备。对于客观存在的设备差异，则引入DeviceTree来抽象描述硬件信息。设备树在启动后被U-Boot传递到内核(通过bootcmd定义的脚本命令传递)，在内核加载时解析设备树创建对应的总线和设备。驱动工程师只要针对硬件修改设备树和实现对应的驱动即可适配不同的硬件，这就简化了开发方式。

注意: **并不是所有设备都是由设备树管理，热插拔设备主要通过总线枚举或者udev机制进行管理。**
...
对于网络上有经验分享或者设备树中已经支持的设备，会告诉你如何添加可以实现功能；可是为什么这样可以实现，却是复杂的问题。对于芯片原厂或外部器件厂商新研发的产品，可能市面上并没有经验分享，又如何通过设备树进行支持。这些疑惑在最初学习了解设备树时一直困扰着我，如果去关注内核dts目录下的所有设备树，会发现每家厂商设备树在框架上基本一致，但在具体的细节上却大相径庭。

理解了NXP的设备树，再去看全志、瑞芯微的设备树，似乎还是有很大障碍。这是正常的，设备树在看似简单的语法下，其实维护一套复杂的描述体系；在内核的版本迭代支持，也在同步更新和迭代，学习并不简单。

设备树在语法有严格的特征限制，可以看到如下基本规则。

1. 根节点以"/"开始，以节点为基础描述硬件
2. 以"{"和"}"定义节点，支持节点的嵌套
3. 以"&"的方式引用节点，不同节点可通过引用进行访问
...
标准属性因为和内核的访问有关，需要有严格的限制才能在内核启动后被成功解析，并在驱动中使用函数访问。如果只是去理解语法，不去掌握背后的与内核的操作接口，那么就会迷惑为什么要这样定义。而自定义私有属性只要符合节点属性定义语法，即[propery]=[value]就可以实现功能；那么理解这些变量，就必须去驱动中找寻相应的实现才能够知晓功能，只看语法意义就不大，很多时候需要参考芯片手册和驱动才能理解含义。设

备树学习的困难之处就在于此，任何一个文档都无法理清楚设备树中所有变量定义的功能需求和逻辑，需要配合着内核API接口、芯片寄存器以及驱动实现去理解。上面说了一大段话，不过你可能还是感觉很抽象，那么这里配合全志的t113_i中的twi，也就是i2c驱动去理解，这里同样进行了备注。
...
可以看到，对于一个外设模块节点，大部分都是标准属性；用来声明模块的硬件功能。那么理解了这些标准属性，就可以更简单的去掌握和开发设备树。

本篇也是以这些标准属性为核心，讲述设备树的语法。
...
- [设备树说明](#设备树说明)
  - [基础语法](#base)
    - [device\_model](#device_model)
    - [include](#include)
    - [node](#node)
    - [node\_path](#node_path)
  - [值属性定义](#properties_definitions)
    - [value\_type](#value_type)
  - [常见属性类型](#property_type)
...
在硬件中，总线(如i2c, spi等)是SOC和外部器件的连接通道, 在其上挂载着各类器件。不过对于内部模块，如gpio、rtc、wdg、csi等设备，是通过AHB或AXI连接到CPU Core上，这并不属于需要配置的总线。不过为了驱动的统一管理，Linux系统中对于这些内部设备，定义了platform总线来管理。

在设备树中，满足以下条件的设备节点会被platform总线管理。

1. 根节点的子节点
2. 根节点的子节点中compatible属性中包含`"simple-bus", "simple-mfd", "isa", "arm,amba-bus"`时，其子节点会被platform总线管理
3. 依次类推，子节点其上层有上述描述，其子节点也会被platform总线管理
...
/ { //根节点
    ...
    soc: soc {
        ...
        aips1: bus@2000000 {
            ...
            uart7: serial@2018000 { // uart7节点由platform管理
...
基于以上信息，DTS/DTSI是基于DTS语法实现的设备描述文件；DTB则是编译后用于内核解析的二进制文件，也是下载时需要使用的文件；dtc则是进行语法文件以及二进制文件之间转换的工具 。

对于设备树在内核中的加载流程如下所示。

1. Uboot从存储中读取dtb文件，加载到内存中并传递。
2. 内核从dtb文件中读取设备树信息，初始化设备树。
3. 内核中通过platform_device解析设备树，在系统中创建设备
4. 驱动中匹配创建的设备，执行驱动probe，创建系统设备文件。

dts语法看起来所见即所得，是一种典型的描述语言，其实内部有很多约束和规定，看起来简单，但适配起来其实相当复杂。其中最基础的就是组合和覆盖规则，也是设备树修改实现的基础。
...
覆盖机制的存在，当包含文件较多时，分析最终输出的状态比较复杂，这时可以通过dtc工具反编译dtb来获取设备树dts源文件，直接从最终源码查看。
...
DTS中#include语法和C语言中类似，支持将包裹的文件直接放置在#include位置从而访问到其它文件的数据。

这里展示在NXP i.MX6ULL设备树中包含dtsi文件得实现。
...
`#include`只是单纯得添加合并文件，因此也可以用来包含dts文件，具体如下所示。
...
对于设备树来说，基本单元就是node，也叫做节点。node从根节点开始，被组织成树状结构，添加不同的设备节点描述的。
...
在节点说明中，`[]`中的均表示可选，最简单得可以定义只有node-name的节点，label主要是方便在设备树文件中引用。对于节点内部，properties definitions则是定义节点的内部属性，child nodes则是定义子节点。

这里举个gpio_keys的节点例子说明。
...
1. 设备树文件都由根节点开始，且只有一个根节点(设备树可以定义多个根节点，但是根节点会合并成一个)，其它所有设备都作为子节点存在，由节点名和一组节点属性构成。
2. 节点属性都是有property-value的键值对来描述，并以";"结束，不过value可以为空。
3. 节点间可以嵌套形成父子关系，这样可以方便描述设备间的关系。
4. 节点名支持name@unit_address的格式，其中后面的unit_address可选，一般为设备地址，这是为了用于保证节点是唯一的标识，当然用其它数字也可以。同时，节点名也支持label:name@unit_address的格式, 这里的label就是节点的标签别名，我们可以&label来直接访问节点。如对于gpio_keys: gpio_keys@0 可以通过&gpio_keys来访问，当对于节点添加或者覆盖属性时，通过此方式选择对应节点。

对于项目来说，不建议直接修改官方SDK提供的dts文件，而自己完整建立设备树，又过于复杂；这里就可以依赖我们之前提到的覆盖和引用机制，设备树提供通过引用label的方式，允许我们在其它文件中修改已存在的节点，或者添加新的节点。基于这些原则，就可以在已有节点追加新的数据。

这里使用如下代码在gpio_keys: gpio_keys@0中增加节点。
...
节点路径是指从根节点到所需节点的完整路径，可以唯一地标识设备树中的节点位置。

不同层次的设备树节点名字可以相同，同层次的设备树节点则必须唯一。例如前面节点的结构参考图中，对于节点gpio_keys的子节点key1，其节点路径就是"/gpio_keys/key"，节点路径也是驱动中访问节点的一种方式。

```c
// key1节点
/{  // 根节点
    gpio_keys: gpio_keys@0 { //gpio_keys节点
        key1@1 {    //key1节点
        }
    }
}
...
设备树中的属性，主要目的用于描述设备的特征，用于驱动查找、访问和配置设备的状态。从设计上来说，设备树的属性主要分为两大类。

1. 通用属性，设备树定义时规定的通用属性，如compatible、model、status等。特征与内核的接口关联，具有确定的功能含义。
2. 私有属性，由厂商和用户自己定义，用于描述硬件的配置信息，如默认状态、配置信息等。

通用属性和私有属性共同提供驱动需要的设备参数和配置信息，这里主要以通用属性来说明设备树中的相关处理和实现。

### compatible

通用属性。compatible属性是值是由特定编程模型的一个或多个字符串组成，用于将驱动和设备连接起来。我们在驱动中也是通过compatible来选择设备树中指定的硬件，是非常重要的属性(当然部分驱动也支持通过节点名称取匹配)。

compatible属性定义的变量格式一般为:`vendor,device`, 其中vendor表示厂商，可选，device则表示指定型号，大部分和节点名称一致(不一致也不影响功能)。
...
在驱动中，of_match_table数组的compatible属性就是用来匹配对应设备节点，基于严格的字符串匹配，驱动内的字符串和设备树中compatible的值之一要保持完全一致。

注意: **对于有具体总线的器件，如i2c，spi，uart等，驱动会匹配相应总线下的设备，对于不存在总线的设备，如I/O，ADC，通过平台总线platform_driver匹配时，需要挂载在根节点或sample-bus等节点上才能被驱动匹配成功。**

这里以spi总线下的icm20608驱动为例，展示compatible属性的匹配方法。

```c
// 设备树节点
spidev: icm20608@0 {
    compatible = "rmk,icm20608";   // 匹配compatible
...
static struct spi_driver icm20608_driver = {
    ...
    .driver = {
            .owner = THIS_MODULE,
            .name = SPI_ICM_NAME,
            .of_match_table = icm20608_of_match,   // 指定设备树匹配列表，使用.compatible属性匹配
    },
};
```

在实践中，驱动中通过of_match_table指定设备树匹配列表，找到指定的节点去访问。

### device_type

通用属性，字符串类型。device_type属性主要用来描述设备的设备类型，早期用于匹配设备；不过目前驱动中主要使用compatible属性来匹配设备，**device_type基本已经废弃了**。
...
1. okay：设备可操作，工作正常。
2. disabled: 设备目前不可操作，不过对于热插拔设备，可以在后续改变为可操作。
3. fail: 出错的，设备不可操作，一般不使用。
4. fail-sss: 表示设备不可运行，目前驱动不支持，待修复。“sss”的值与具体的设备相关，一般不使用。
...
通用属性，可编码数组类型。reg属性是驱动中最常用的属性之一，是设备树和硬件关联的重要属性，包含硬件模块对应的寄存器地址和长度信息。

reg的定义分为两部分，其中cells属性用于定义子节点reg属性内部的分组，reg则定义寄存器的具体内容。
...
- #interrupt-cells：定义子节点的interrupts属性中中断配置的数量。
  - #interrupt-cells=<1>, 表示子节点的interrupt属性只需要1个cell，来表明使用哪个中断
  - #interrupt-cells=<2>, 表示子节点的interrupt属性需要2个cell，分别来表明使用哪个中断，以及中断触发的类型
...
通用属性。引脚相关的属性有pinctrl-x, pinctrl-names和xxx-gpios功能。

对于NXP的芯片，需要对于所有使用的GPIO指定引脚功能，无论是普通GPIO，还是复用成其它外设。而对于RK3568这类芯片，仅复用引脚时才定义成相应的类型。
...
通用属性。empty或者child-bus-address、parent-bus-address、length类型，主要为处理子节点的reg属性。

ranges属性值为空值，说明子节点继承父节点的地址域，使用相同的起始物理地址。
...
0x2140000
 │
 ├─jr0 0x1000 - 0x2141000
 ├─jr1 0x2000 - 0x2142000
 └─jr2 0x3000 - 0x2143000
```

从上可知，sec_jr1的信息如下。

- 寄存器地址占1位，长度1位。
- reg_addr = 0x2000，size = 0x1000。

其父节点支持range属性，且需要转换，则sec_jr1的实际物理地址计算如下。
...
2. io-channel-ranges: 继承了当前节点的子节点可以引用当前节点的 IIO channel。
...
&adc1 {
    ...
    io-channel-ranges;                                   //继承了当前节点的子节点可以引用当前节点的 IIO channel
    //...
};
...
对于嵌入式Linux设备，语法树是在/sys/firmware/devicetree下，可使用如下命令显示。
...
至此，对设备树的语法进行了初步的讲解，当然在实际驱动开发中，熟悉这些知识还是不够的。日常打交道还有很多是芯片厂商或者方案商定义的具有特定功能的自定义私有属性键值对，这就需要长期的积累。不过理解了设备树语法的原理，反过来去理解这些自定义私有属性，就比较清晰明了的。

...
- VMvare 15.5.7/wsl2
- Ubuntu 20.04 LTS
  - 下载地址:<https://mirrors.tuna.tsinghua.edu.cn/ubuntu-releases/20.04/>

说起来，因为项目平台不同，我使用过的虚拟机如VisualBox，VMvare以Hyper-V，从我的体验来说**wsl2+vscode+git**组合是最方便快捷的程序开发和维护方案。不过wsl2中对于网络默认是NAT方式，对于桥接模式的快捷配置只有windows11才支持，而我使用的是windows10，所以在验证阶段使用tftp和nfs这种网络加载内核和文件系统的方式对于wsl2就不怎么适用；而Hyper-V+Ubuntu的组合，使用来没有VMvare, 文件和数据直接拷贝就能在虚拟机中使用还是方便的。

至于VMvare 15.5.7+Ubuntu 22.04组合时，安装VMTools出现dump core问题，需要更新版本的VMvare可以解决，不过我手上并没有。虽然通过ssh或者samba也能实现跨系统文件复制，但不能直接拖到系统里，也不能使用剪切板直接复制粘贴，使用起来体验很差。这也是我最终使用**VMvare 15.5.7+Ubuntu 20.04 LTS**作为前期产品方案构建平台的原因，平台稳定后则主要使用ssh将更新的应用替换开发板中的应用，这时就将**wsl2+Ubuntu 20.04 LTS**作为应用开发平台。当然这是我的选择，只要能实现目的，其它平台也可以。如果时间紧，那么建议就直接使用

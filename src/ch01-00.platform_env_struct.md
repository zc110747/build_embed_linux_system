...
上述这些平台可以说是典型的支持嵌入式Linux的芯片，可以满足大部分的嵌入式应用功能需求，不过对于国内芯片普遍面临资料不齐全的问题，对于个人开发者需要自己摸索，在学习阶段会面临更多跳转，需要慎重考虑。
...
开发环境的构建，以我的经历来说是感受不到趣味性的，一般都是参考网上分享的教程和官方的SDK结构摸索着进行学习的。最初只知道跟着敲命令，如果和教程表现一致还好，可当你使用相同版本的系统，却因为缺少某个库文件或权限问题而执行失败后；在不理解背后原理的情况下，只能在网络上搜索相似的问题，看有没有解决办法（现在可以求助AI），往往发生构建错误浪费1-2个小时仍然没有解决。特别是交叉编译时，按照教程却卡在某一步，经常枯坐一晚上不停重试，挫败感明显。

从我的学习经历看，这似乎是必须经历的过程，不过我还是希望这份经验总结下来，既是对自己过去的总结，也是希望对后来者有所帮助。这里说明下构建Linux平台时主要遇到的问题。
...
在实践中主要遇到perl和python的版本问题。有些比较老的软件使用python2语法的脚本用于处理，而对于新版本的系统，已经移除了python2，甚至连apt源中都已经删除；以我的经历不建议去手动安装，会遇到很多兼容性问题；我都是直接修改软件中的脚本源码来支持python3(曾经系统学习过python的语法)，预处理的脚本大都很简单，并不复杂。另外就是脚本中缺少库导致import失败，这类需要使用脚本语言的本身安装的支持的库安装才能成功执行，例如编译依赖python3的第三方库，就需要使用pip安装。

- 版本中某类库或者软件版本和应用不匹配，这种问题可以分两类处理。

1. 编译使用软件版本新版本，搭配较老的系统(如Ubuntu16)。使用编译或者deb安装最新的包可能会遇到(apt中会适配当前版本，不会遇到此情况)，这类情况最优建议是使用更新的Ubuntu系统或者去找老版本的软件，不建议直接去更新系统的库(特别是/lib和/usr/lib下的库文件)。我曾经尝试去同步升级相关的系统库，结果因为某些软件和更新的系统库不兼容，导致系统程序崩溃，最后只能无奈重装。

...
如果虚拟机系统内资料并不是太需要保护，那么为了后期命令的方便，可以使用短密码(确保PC不会被他人随意访问，且内部没有重要信息)，否则还是建议使用长密码。另外如果硬盘空间足够，建议虚拟机的空间分配至少50G以上，并且越大越好，虽然后续通过VMware的扩展功能可以增加硬盘空间，不过没有原始的全范围空间使用方便。
...
sudo apt-get install subversion zlib1g-dev gawk gettext libelf-dev ecj fastjar bzip2 libgmp-dev libmpc-dev -y
```

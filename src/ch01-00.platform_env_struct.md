...
嵌入式Linux的学习需要软硬件结合学习，本系列将以**NXP i.MX6ULL**和**全志H616**作为基础平台去验证。选择这两个平台的主要是因为我刚好购买了这两块开发板，而且这两块板子分别为32位和64位ARM芯片，也算比较主流的开发平台。
...
1. 访问权限问题，很多人可能直接使用root权限规避这个问题，不过root权限一些指令容易损坏系统，误删除或修改关键文件，导致系统损坏，不建议使用。不过使用一般用户，则很容易面临权限不足问题，这是不可避免的，例如mkdir，vim，touch等命令创建的文件，都默认设置为当前用户管理，如果在root权限下创建，普通用户就不可能更改权限等。另外文件从windows迁移到Linux很可能丢失权限，这个需要注意。
...
3. 当然比较棘手的问题是版本中某类库或者软件版本和应用不匹配，这种问题分为两类.

- 编译使用软件版本新版本，搭配较老的系统(如Ubuntu16)。使用编译或者deb安装最新的包可能会遇到(apt中会适配当前版本，不会遇到)，这类情况最优建议是使用更新的Ubuntu系统或者去找老版本的软件，不建议去更新系统的库(特别是/lib和/usr/lib下的库文件)。我曾经尝试去同步升级相关的依赖库，结果因为其它软件和库不兼容，导致系统只能重装.

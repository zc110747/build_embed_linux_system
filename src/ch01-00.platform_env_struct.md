...
嵌入式Linux的学习需要软硬件结合学习，本系列涉及**NXP i.MX6ULL**、**全志T113-i**、**瑞芯微RK3568**等作为基础平台去验证。选择这些平台的原因也很简单，一是我有这些开发板，不需要额外花费；另外这些板子包含低端和中高端的应用场景，分别对应低端的工业平台和中高端的视频、AI算法平台，算目前比较常规的应用。
...
- NXP i.MX6ULL，**Cortex-A7内核，32位单核处理器**，典型的低端SOC，适合运行无桌面Linux系统或基于QT环境的界面开发。NXP开源资料丰富，适合去系统的学习嵌入式Linux系统，不过因为性能限制，主要做基于C/C++的数据处理以及局域网内少量访问的嵌入式应用，对于跑图形化系统、Android应用或者音视频处理则明显性能不足。
- 全志T113-i, **Cortex-A7双核，另外带Cortex-M0的MCU**。支持Linux开发以及基于rt-thread的双系统混合开发方式，这也逐渐成为SOC设计的主流方式。其中Cortex-A7核可以跑图形界面处理，而Cortex-M0核又能兼顾实时性执行电机处理等应用，从而广泛应用于工业领域。
- 瑞芯微RK3568, **Cortex-A55，64位4核处理器，带GPU和NPU**，中高端SOC，适合运行Debian或Android11以上的图形化系统。平台上可以适配Python、Go、JS、Java(JNI)等上位机语言开发，性能满足大部分音视频、图像处理应用以及AI应用。

上述这些平台可以说是典型的支持嵌入式Linux的芯片，可以满足大部分的嵌入式应用功能需求。不过对于国内芯片普遍面临资料不齐全的问题，对于个人开发者需要自己摸索，在学习阶段会面临更多挑战，需要慎重考虑。

考虑产品的构建环境，本系列将以Ubuntu 22.04 LTS作为开发环境，其它系统版本理论也可以；不过每个版本遇到的问题可能不一致，就需要遇到时自己去分析解决。这个也是嵌入式学习的难点，每个人遇到的问题都不一样，需要积累经验去总结解决。

在嵌入式系统学习的过程中，开发环境构建是有门槛的，**不建议入门过程中在开发环境构建花费大量时间**。以我的经历来说，能够熟练构建一套快速开发环境(编译工具的移植、库的交叉编译、脚本构建)，在整个嵌入式Linux系统学习中都是比较困难的部分。很多方案商和各类开发板厂商，其最核心业务就是**在芯片厂商的方案基础上，基于自己设计的硬件平台实现一套开发环境和启动方案，提供技术资料和技术支持服务**；这里面很多技术都需要前置知识点，不建议在入门时就扎入这个深坑。

开发环境的构建，以我的经历来说是感受不到趣味性的，一般都是参考网上分享的教程和官方的SDK结构摸索着进行学习的。最初只知道跟着敲命令，如果和教程表现一致还好，可当你使用相同版本的系统，却因为缺少某个库文件或权限问题而执行失败后；在不理解背后原理的情况下，只能在网络上搜索相似的问题，看有没有解决办法（求助AI后，这个问题已经大大缓解），往往发生构建错误浪费1-2个小时仍然没有解决。特别是交叉编译时，按照教程却卡在某一步，经常枯坐一晚上不停重试，挫败感明显。

从我的学习经历看，这似乎是必须经历的过程；不过我还是希望这份经验总结下来，既是对自己过去的总结，也是希望对后来者有所帮助。

这里主要说明下构建Linux平台时主要遇到的问题。

📚 访问权限问题，这是Linux系统最常见的问题之一

很多人可能直接使用root权限规避这个问题，不过root权限一些指令容易损坏系统文件，误删除或修改关键文件都会导致系统损坏。另外部分厂商的SDK会对root下运行进行限制，则根本无法使用root进行编译。而使用一般用户，则很容易面临权限不足问题，这基本是很难避免的。类似mkdir，vim，touch，install等命令创建的文件或目录时，都默认设置为当前用户管理。而如果是在root权限下创建，普通用户可能就不会有修改权限；另外文件从windows迁移到Linux很可能丢失权限；这个时候就需要使用chmod(修改文件的读写权限)、chown(更改文件的所属用户)等命令，而不要想着就在root下处理，可能对后续带来很多麻烦。

📚 缺少某类运行支持库

表现为编译时缺少某个头文件([file].h不存在)或者链接中-l[file], 运行时缺少lib[file].so。这时分两种情况，一种是编译本系统运行的文件或工具(运行在桌面端)，这时候通过**apt --names-only search [file]**从安装源中查找，在通过apt-get install安装接口即可。还有一类是交叉编译时缺少某个类库，这时候就比较麻烦；需要去找到相应的类库源码，编译安装的特定目录，在编译时引入此目录，或者使用debian，ubuntu之类的嵌入式系统，在chroot或者应用环境下使用apt软件安装。

📚 系统脚本语言环境与程序中处理的脚本不兼容或者不支持某些类型的库

在实践中主要遇到perl和python的版本问题。有些比较老的软件使用python2语法的脚本用于处理，而对于新版本的系统，已经移除了python2，甚至连apt源中都已经删除；以我的经历不建议去手动安装，会遇到很多兼容性问题；我都是直接修改软件中的脚本源码来支持python3(学习过python的语法，不过现在可以直接让AI给出解决办法)，预处理的脚本大都很简单，并不复杂。另外就是脚本中缺少库导致import失败，这类需要使用脚本语言的本身安装的支持的库安装才能成功执行，例如编译依赖python3的第三方库，就需要使用pip安装。

📚 版本中某类库或者软件版本和应用不匹配，这种问题可以分两类处理。

1. 编译使用软件较新版本(老版本不在支持)，搭配较老的系统(如Ubuntu16)。使用编译或者deb安装最新的包可能会遇到(apt中会适配当前版本，不会遇到此情况)。这类情况最优建议是使用更新的Ubuntu系统或者去找老版本的软件，不建议直接去更新系统的库(特别是/lib和/usr/lib下的库文件)。我曾经尝试去同步升级相关的系统库，结果因为某些软件和更新的系统库不兼容，导致系统程序崩溃，最后只能无奈重装。
2. 软件已经放弃更新，但系统使用新版本；这种有能力的话可以去更改软件适配新版本库，否则的话建议去寻找替代的产品，不建议因为某款软件去回退系统版本。

注意: **在构建Linux系统平台时，要优先确认自己的Linux平台和版本，不同系统和版本，其适配和处理的问题差异很大。没有能力解决时就选择官方SDK提供的系统版本，这也是我多次在不同平台上构建得出的是最重要的经验。**
...
注意: **类似ChatGPT、Gemini、Deepseek，trae等语言类AI工具的发展，十分方便检索问题和解决办法，在学习中一定要善于使用。**
...
Ubuntu默认使用的镜像源在国外，直接下载访问比较慢，大部分都会直接失败，所以我一般会更新使用国内源。

目前使用的是国内清华镜像源(也可以参考其它镜像源，如阿里、华为等，可自行搜索)。
...
构建完成后，需要安装编译支持的脚本或者库。下面提供了一些在编译驱动，内核还有一些第三方库或软件时需要增加和更新的库和程序。

这些库和程序是在各类编译中整理的，可以直接执行指令进行安装，能够解决大部分编译或者交叉编译时遇到的问题。
...
对于Linux平台环境的构建，包含熟悉Linux平台命令和文件知识，安装方便使用和开发的工具，能够自己编译或交叉编译些库和程序。

这部分工作并不困难，却十分复杂，不同系统(centos，ubuntu)，相同系统的不同版本(16.04，22.04)，甚至相同版本系统下不同的库支持情况，都会面临不同的问题。这部分不建议机械的学习，而是有概念后，在使用Linux的过程中，积累深入总结，当你能够整理出自己的笔记或者能实现自己的一键构建工具后，也就自然学会掌握了。
...
- <https://gitlab.arm.com/tooling/gnu-toolchains-for-arm/-/tree/releases/15.3.rel1?ref_type=heads>
...
- 编译目的平台：arm位32位arm，aarch64为64位arm；aarch64一般只有中高端的arm平台才支持，如Cortex-A53等
- 厂商信息：可省略，arm提供的一般为none.
...
wget https://gitlab.arm.com/api/v4/projects/tooling%2Fgnu-toolchains-for-arm/packages/generic/gnu-toolchain/15.3.rel1/arm-gnu-toolchain-15.3.rel1-x86_64-aarch64-none-linux-gnu.tar.xz
...
对于嵌入式Linux开发，在Windows下有更方便的开发方法，那就是使用WSL2开发。这里不推荐使用WSL1，WSL1使用在Windows下模拟Linux的API，目前微软基本放弃更新，且有接口和应用的缺失，如系统不支持消息队列、不支持systemd、编译内核时缺少某些头文件和库，特别是交叉编译时会遇到各种莫名其妙的问题，而且很难找到原因。

注意: **对于wsl2，代码和环境都要构建在linux目录下，而非挂载的/mnt/d这种windows目录下，一个是跨文件系统访问速度极慢，另一个是windows下大小写不敏感，一些rm命令会直接删除应用目录。**

对于WSL2则完全采用Hyper-V虚拟机方式，目前除网络和USB支持有问题外，和VMVare使用基本一致。另外WSL2和老版本的VMVare对于虚拟网络有兼容性问题，会导致VMVare无法启动系统，需要将VMVare升级到15.5.7版本或者更高版本。
...
注意：WSL2虽然默认支持通过/mnt/*方式访问windows平台的文件，不过这是通过文件系统转译的方式访问的，效率低，会大幅度增加编译时间。另外windows文件**不区分大小写**，即使通过Linux系统访问也会有这类不一致问题，导致处理出错，从而有可能误删除文件。在使用WSL2时，还是移动到WSL2的Linux目录下使用(非'/mnt/'访问的目录)，**尽量避免直接使用windows系统下文件**。

### WSL2的系统迁移

wsl默认在C盘的Windows目录下，如果需要移动到其它目录，可通过如下方法迁移wsl到其它目录的方法。
...
对于wsl2环境构建，包含I.MAX6ULL、RK3568、H618芯片的驱动、内核、库编译、wsl2图形环境xfce远程连接、qtcreator和qt compiler环境支持、以及驱动开发、验证下来wsl2可以满足开发的需求。

不过wsl2还是有一些限制，不过可以通过其它方法绕开。

...
开发环境的构建，以我的经历来说是感受不到趣味性的；我是参考网上分享的教程已经官方的SDK结构摸索着进行学习的，最初只知道跟着敲命令，如果和教程表现一致还好，可当你使用相同版本的系统，却因为缺少某个库文件或权限问题而执行失败后，因为不理解背后的原理，只能在网络上一遍遍搜索相似的问题，看有没有解决办法，往往发生构建错误浪费1-2个小时仍然没有解决。特别是交叉编译时，按照教程却卡在某一步，经常枯坐一晚上不停重试，挫败感明显，虽然这是必须经历的过程，但有其它方面的触类旁通后可以事半功倍。建议在有一定技术积累后再进行。不过在这个过程中我也总结了如下经验，希望对后来者有所帮助。

...
上述两个平台可以说是典型的支持嵌入式Linux的芯片，基本可以满足大部分的嵌入式应用功能需求。考虑两款产品的构建环境，本系列将以Ubuntu 22.04 LTS作为开发环境，其它系统版本理论也可以，不过每个版本遇到的问题可能不一致，就需要遇到时自己去分析解决。
...
1. 访问权限问题，很多人可能直接使用root权限规避这个问题，不过root权限一些指令容易损坏系统，误删除或修改关键文件，导致系统损坏，不建议使用。但使用一般用户，如果在操作时不注意，很容易面临权限问题，这是不可避免的，例如mkdir，vim，touch等命令创建的文件，都属于当前用户管理的目录，如果在root权限下创建，普通用户就不可能更改权限等。另外文件从windows迁移到Linux很可能丢失权限，这个需要注意。
...
- 编译使用软件版本最近的版本，搭配较老的系统(如Ubuntu12)。使用编译或者deb安装最新的包可能会遇到(apt中会适配当前版本，基本不会遇到)，这类情况最优建议是使用更新的Ubuntu系统或者去找老版本的软件，不建议去更新系统的库(特别是/lib和/usr/lib下的库文件).我曾经尝试去同步升级相关的依赖库，结果因为其它软件和库不兼容，系统最后只能重装.
...
对于wsl2环境，测试I.MAX6ull, RK3568, H618芯片的驱动，内核，支持库编译，wsl2图形环境xfce远程连接，qtcreator和qt compiler环境支持，以及驱动开发，验证下来wsl2可以满足开发的需求。不过wsl2还是有一些限制，不过可以通过其它方法绕开。
...
- wsl2网卡桥接模式，需要windows11才支持，windows10配置桥接会导致无法连接网络
  ...
  - 解决办法: 对于tftp协议使用windows下的**Tftp64**服务器软件, 支持tftp协议，可以直接访问wsl的接口。对于nfs则使用hanewin nfsd支持nfs v2，v3的访问。

这是我使用wsl2遇到的主要问题，其它问题都能够解决。总之，wsl2我认为是比较完善的Linux环境，能够满足应用，以及驱动开发的需求。

...
嵌入式Linux的学习一定是要配合硬件平台的，本系列将以**正点原子的阿尔法开发板 NXP i.MX6ULL**和**核桃派 全志H616**作为基础平台去验证。选择这两个平台的主要原因是因为我刚好购买了这两块开发板，而且这两块板子分别为32位和64位ARM芯片，也算比较主流的开发平台。
...
开发环境的构建，以我的经历来说是感受不到趣味性的；我是参考正点原子还有一些网上分享的教程摸索着进行学习的，最初只知道跟着敲命令，如果和教程表现一致成功还好，可当你使用相同版本的系统，却因为缺少某个库文件或权限问题而执行某个命令失败后，因为不知道背景和原理，只能去搜索相似的问题，看有没有解决办法，当发生构建错误浪费1-2个小时仍然没有解决，特别是交叉编译时，按照教程却卡在某一步，经常枯坐一晚上不停重试，挫败感明显，这是必须经历的过程，但有其它方面的触类旁通后可以事半功倍，所以建议在有一定积累后再去进行。不过在这个过程中我也总结了如下经验，希望对后来者有所帮助。

1. 权限问题，建议从应用放置的初始目录修改为0755(用户目录，不要修改系统目录如bin/，usr/bin等)，而不是切换到root权限，否则后续会带来很多问题，另外文件从windows迁移到Linux很可能丢失权限，这个需要注意。
2. 缺少某类库，编译时表现为缺少某个头文件([file].h不存在)或者链接中-l[file], 运行时缺少lib[file].so, 这时候通过**apt --names-only search [file]**从安装源中查找，在通过apt-get install安装。
3. 当然比较棘手的问题是版本中某类库或者软件版本和应用不匹配，分为两类，一种是编译使用软件版本最近的版本，搭配较老的系统(如Ubuntu12), 而通过编译或者deb安装最新的包可能会遇到(apt中会适配当前版本，基本不会遇到)，这类情况最优建议是使用更新的Ubuntu系统或者去找老版本的软件，不建议去更新系统的库，我曾经尝试去同步升级相关的依赖库，结果因为其它软件和库不兼容，导致系统最后只能重装，一种是软件已经放弃更新，但系统已经使用新版本，这种有能力的话可以去更改软件适配新版本，否则的话建议去寻找替代的产品，不建议因为某款软件去回退系统版本。
...
对于构建Linux系统平台，没有一定的基础去修改是比较困难，如果错误的使用root权限去修改系统配置，是有几率导致系统功能失效，严重的会崩溃无法启动，需要重新安装。这部分技术十分杂乱，没有任何教程能覆盖到所有的内容。不提Linux本身的命令，对于用户添加的应用如net-tools，ssh，opencv，python，node，sqlite，tftp，每个软件都有一套命令机制和配置方法，只是单纯去学习意义不大，而且如果更换了系统版本，对于同一个软件也可能遇到不同的问题。这部分不适合专门去学习，而应该日常使用中总结。

...
开发环境的构建，以我的经历来说是感受不到趣味性的；我是参考正点原子还有一些网上分享的教程摸索着进行学习的，最初只知道跟着敲命令，如果和教程表现一致成功还好，可当你使用相同版本的系统，却因为缺少某个库文件或权限问题而执行某个命令失败后，因为不知道背景和原理，只能去搜索相似的问题，看有没有解决办法，当发生构建错误浪费1-2个小时仍然没有解决，特别是交叉编译时，按照教程却卡在某一步，经常枯坐一晚上不停重试，这种挫败感特别难受。当然在这个过程中我也总结了如下经验，也算有所收获。

1. 对于权限问题，建议从应用放置的初始目录修改为0755(用户目录，不要修改系统目录如bin/，usr/bin等)，而不是切换到root权限，否则后续会带来很多问题，另外文件从windows迁移到Linux很可能丢失权限，这个需要修改。
2. 对于缺少某类库，编译时表现为缺少某个头文件或者链接中-l[file], 运行时缺少lib[file].so, 这时候通过**apt --names-only search package**从系统库中查找，在通过apt-get install安装。
3. 另一个比较棘手的问题是版本中某类库或者软件版本过低，如果使用较老的系统(如Ubuntu12), 而通过编译或者deb安装最新的包可能会遇到(apt中会适配当前版本，基本不会遇到)，这类情况最优建议是使用更新的Ubuntu系统或者去找老版本的应用，不建议去更新系统的库，我曾经尝试去同步升级相关的依赖库，结果因为其它软件和库不兼容，导致系统最后只能重装。

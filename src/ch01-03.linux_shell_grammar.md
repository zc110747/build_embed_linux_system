...
在Linux平台进行编译时，解决问题重要难题就是需要理解项目的相关脚本。其需要理解和掌握shell语法、掌握shell语法，而且这部分也是自动化构建系统实现的必备知识。

对于完整的shell语法，是不可能一篇文章讲完，甚至一本书也是远远不够。另外相关知识也会随着系统迭代，产生相应变化；本节只能算是进行大致的说明，为后续学习打下基础。
...
shell是用户与Linux系统间接口的程序，它允许用户向操作系统输入需要执行的命令。这不仅是一套指令语法，还包含扩展的程序和命令功能，只能依靠日常使用中积累掌握。

作为shell语法的总结，本文也参考如下资料进行整理，可以配套学习了解。

- **<Linux命令行与shell脚本编程大全>**
- **<Linux Shell核心编程指南>**

在Linux日常使用中，都是通过命令来完成文件创建、下载、解压、处理、删除等工作。不过在维护Linux平台时，通过一步步的手动指令操作来完成工作就显得繁琐。为了统一处理这类问题，Linux下环境支持一整套shell语法来实现流程化操作；类似windows下的BAT和PowerShell语法，可以通过脚本来管理日常任务。

shell语法包含指令和脚本语法操作，其中指令可以参考之前介绍。

- [Linux Shell命令行总结](./ch01-01.linux_shell_cmd.md)

脚本语法则在本节进行说明，首先描述下shell内部的一些特殊说明。

- 在命令行中，双引号和单引号都表示引用一个整体，不过单引号可以屏蔽转译字符。
- shell语法与所在bash环境息息相关，**不同的bash环境可能支持不同的语法**。例如bash、sh、zsh等支持的语法就有些差异。另外嵌入式端bash往往支持的语法有限，在实际使用中如果语法执行出错，要考虑执行环境的差别（如busybox支持的语法只支持少部分语法)。
...
系统环境变量是在操作系统中全局存在的变量，为系统和应用程序提供了重要的配置信息和运行环境；例如命令和系统库检索查找的地址、用户工作目录等。

在命令行或者脚本中，可通过`$name`的形式获取环境变量的具体值，也可通过`echo $name`打印环境变量的值。

另外通过printenv可以查看当前用户下的所有环境变量，或`printenv $name`打印具体环境变量的值。
...
对于用户环境变量，可通过export命令导入通过命令行或者shell脚本导入。
...
对于变量内容，一般为引号包裹的字符串，其中引号类型可以是`双引号("")或单引号('')`。

其中双引号("")表示一个整体，单引号('')则在此基础上还会屏蔽特殊符号。对于字符串中的引号还包含反引号(``)，表示执行命令。
...
局部变量同样是脚本语法中重要的部分；配置定义、状态管理、数据传递等处理，都依赖于局部变量进行存储和管理。
...
在shell环境中，支持以下方式进行数学或者逻辑运算。

- 使用expr命令，直接进行数学或者逻辑运算，如`expr 2 + 2`。
- 使用bc命令，通过管道符将数据导入到bc命令中进行运算，例如`echo "scale=4; 3.44/5" | bc`。
- 使用`$和$[operation]`格式的方括号进行数学运算，例如`$[2+2]`，或者直接`[operation]`配合if、while进行逻辑运算。
- 使用`$和$((operation))`格式的双括号进行数学运算，例如`$((2+2))`。
...
前面讲述了支持数学运算的常用命令和格式。对于运算语句，其主要支持的运算符包含算术运算符、比较运算符和逻辑运算符；可以用于数值计算，逻辑分支处理和循环语句中，是实现shell脚本的基础之一，具体说明如下所示。
...
对于上述语句，其中逻辑运算符`与(&&)`和`或(||)`执行需要进一步说明，具体内容如下所示。

- 与`&&`：and语句，满足前一个后续的才会执行，当任意一个不满足即退出执行，并作为状态返回，全部满足则返回true。
- 或`||`：or语句，前一个不满足后一个才会执行，当任意一个满足即退出执行，并作为状态返回，全部都不满足则返回false。
...
在shell环境下，默认会将数据和错误输出到屏幕上显示；在某些情况下，希望把调试打印信息隐藏或者保存起来，这时就可以使用输出重定向，将数据和信息通过其它通道进行处理，如文件、管道符或者其它通讯接口中。

输出重定向是常用的打印信息处理方式，常用的输出重定向符号和功能如下所示。

- 使用"符号 >"重定向输出内容，如果指定为文件且已经存在，此时会覆盖该文件重新生成(之前数据丢失)。
- 使用"符号 >>"重定向输出内容，如果指定为文件，文件不存在则创建，存在则追加数据。

对于重定向"符号 >和>>"，也支持通过符号&、数字来实现组合逻辑。

- 使用"符号 &>"可以将标准输出和错误数据定向到同一文件。
- 使用"符号 &>>"可以追加数据的方式定向标准输出和错误数据。
- 使用"符号 2>或符号 2>>"可以将错误数据输出重定向文件，另外可以组合使用"符号 2>&1"，将错误数据通道重定向标准输出通道。

这里涉及到命令执行的知识点，命令在执行后会进行打印输出，一般标准输出在通道1中，错误输出在通道2中；如果只是使用">和>>"重定义，正确的打印信息会被重定义，错误的输出仍然会被打印输出，此时使用"&>"或"&>>"进行定向处理，就可以全部进行输出。

注意: `/dev/null`文件是shell环境提供的进行数据丢弃的文件。数据不希望显示和处理，将其导入到/dev/null中。

关于重定向逻辑，具体示例如下所示。

```shell
# 将date的输出写入datefile.txt
date>datefile.txt

# 将date的输出重定向追加到datefile.txt中
date>>datefile.txt

# 将标准输出和错误输出定向到不同文件
# 标准输出重定向不带数字
# 错误输出重定向使用格式:"cmd 2>err.file"
ls -l /etc/hosts /nofile 1>test.log 2>err.log
...
和输出重定向相反，shell命令中可以使用符号<和<<从文件中读取数据，并传输到命令中作为输入执行，这种操作被称为输入重定向。

其中符号<和<<都支持从文件中读取数据，不过对于<<在此基础上还支持从数据段中读取信息，具体格式如下所示
...
# 读取字符串内容并输出到1.txt文件中
cat <<< "this is a test!" > 1.txt
```

对于输入重定向来说，其中选项的说明如下所示。

- command指的是常用处理文件的命令，如cat、less、more等。
- filename就是处理的文件名称
- delimiter则是用于处理保存文件数据段的分隔符，包含起始位置和结束位置，成对出现。可以是EOF、BLOCK、SEQ等任意字符串，只要不和内部数据段重复即可，不过常见使用EOF进行处理。

## 管道标识符(\|)

Linux的管道，可以将命令的输出结果，存储到管道中，然后让后续命令从管道读取数据，进行进一步处理。

对于管道，通过`|`连接。如果命令需要顺序执行，可以通过先写入文件，在读取文件访问，当然也可以优化为管道使用。使用管道命令流时不要重复使用相同的文件名(可能在开始时就被覆盖)。
...
不过了解了这些语法，并不表示就已经完全掌握；可以去查看Github开源项目中shell脚本，理解起来还是很困难。这是因为如果一个项目比较小，shell脚本有些杀鸡用牛刀，往往不会使用。项目需要shell脚本管理时，就已经足够复杂了；此时shell脚本覆盖这些复杂度，往往就需要一层层抽象。只是学习这些基础语法就去理解，相当于拿学生的知识理解教授级别的难题，中间差了几个档次，觉得shell脚本晦涩难以理解的原因正是在于此。那么有什么好办法呢？就是自己去实践构建些项目，从简单到复杂，基础就是简单的处理文件，执行编译、复制的操作，复杂一些可以构建测试、安装、配置、运行、打包、清理等SDK方案，只有多加练习，才能真正掌握shell脚本语法。

...
和单片机的流程类似，当有外部触发信号到达，并且所有中断相关的使能都打开的情况下，中断控制器GIC就会根据配置好的硬件信息，将IRQ(或FIQ)的中断触发信息告知指定的Core。处理器感知到该信号后到达时，对于进行irq模式前的系统状态值如cpsr寄存器值，PC指针进行保存(分别保存到SPSR和LR寄存器中)，置位相应的中断状态标志位。计算中断向量的入口位置，将PC设置为该值并跳转。
...
// @node: 设备树节点信息
// @index: 在设备树interrupt属性中的编号(多个中断，单个使用0)
// 返回：对应的中断线号
unsigned int irq_of_parse_and_map(struct device_node *node, int index);

// 从内核中申请中断资源，并注册中断函数
// @irq: 中断号（IRQ Number），通常由设备树中获取
// @handler：中断处理上半部函数，内部不能有延时和让系统进入休眠状态的代码
// @flags: 支持的中断属性配置项
// @name: 中断名称，用于/proc/interrupts显示以及调试
// @dev: 设备私有数据指针，会传递给handler
// 返回: 0：中断申请成功，负值: 申请失败
...
// 带内核管理申请中断资源，并注册中断函数，支持自动释放
// @dev: 管理中断资源的设备，为中断所在的设备节点
// @irq: 中断号（IRQ Number），通常由设备树中获取
// @handler：中断处理上半部函数，内部不能有延时和让系统进入休眠状态的代码
// @flags: 支持的中断属性配置项
// @name: 中断名称，用于/proc/interrupts显示以及调试
...
// 带内核管理申请中断资源，并注册中断函数上半部和下半部，支持自动释放
// @dev: 管理中断资源的设备，为中断所在的设备节点
// @irq: 中断号（IRQ Number），通常由设备树中获取
// @handler：中断处理上半部函数，内部不能有延时和让系统进入休眠状态的代码，可以为NULL表示不需要上半部处理
...
在Linux内核中，中断处理分为上半部（top half）和下半部（bottom half）。其中上半部就是内核的中断函数，主要处理紧急的、对时间要求严格的任务；而下半部则没有限制，可以处理相对不紧急的任务。

对于Linux驱动来说，下半部可以通过多种机制实现，主要包括softirq(软中断)、tasklet、workqueue(工作队列)、threaded irq(带线程化下半部的中断)和timer(软件定时器)。

### 软中断(softirq)

软中断是一种在中断上下文执行的下半部机制。软中断可以在多个CPU上并行执行，因此适用于对响应时间要求较高的任务。

另外softirq运行在原子上下文（atomic context），没有可调度的进程上下文，不能够休眠。对于需要I2C/SPI访问、mutex、延时、阻塞等待的工作，都应该放到workqueue或threaded IRQ中，需要在开发时注意。
...
tasklet是Linux内核Bottom Half(下半部）机制的一种，用于处理中断上下文中不能直接完成的工作。tasklet也是运行于软中断上下文中，因此同样是不能够使用可能导致CPU内核休眠的接口，使用时需要注意。
...
工作队列是一种在进程上下文执行的下半部机制。

工作队列适用于需要睡眠的任务，因为它们在进程上下文中执行，可以安全地调用睡眠函数。
...
### 带线程化下半部的中断(threaded irq)

对于传统的中断处理函数运行在硬中断上下文（Hard IRQ Context），要求执行时间尽可能短，不能执行可能阻塞的操作；如msleep、mutex_lock、i2c_transfer等接口。而在应用中，这些接口往往是硬需求。

Linux提供了threaded IRQ（线程化中断）机制来解决这个问题，使用`devm_request_threaded_irq`来实现功能。

关于线程化下半部的中断接口，具体示例如下所示。
...
内核定时器基于硬件定时器实现，它使用一个全局的定时器链表来管理所有的内核定时器。

当一个定时器被创建并启动后，它会被插入到定时器链表中，并根据其到期时间进行排序。当硬件定时器触发中断时，内核会检查定时器链表，找出所有到期的定时器，并执行相应的处理函数。
...
| 机制 | 执行环境 | 能否睡眠 | 应用 |
| request_irq | hard IRQ | ❌ | 简单快速中断 |
| softirq | softirq | ❌ | 网络、内核高速处理 |
| tasklet | softirq | ❌ | 轻量延迟处理 |
| timer | softirq | ❌ | 周期任务 |
| workqueue | process context | ✅ | 耗时任务 |
| threaded IRQ | kernel thread | ✅ | 设备中断 |

## 总结说明

至此，我们对嵌入式应用中的中断机制进行了初步理解，当然这些都是理论知识的说明，对于嵌入式应用中，如何结合实际情况，配置合适的中断优先级，并实现应用从而满足产品的需求才是最重要的部分。

对于中断来说，需要掌握的是SOC中如何触发、内核中如何管理、设备树中如何添加、驱动中如何解析和注册，已经中断的上下半部处理的方法和注意点。掌握了这些，在开发中旧可以更快速的学以致用，知其然亦知其所以然。

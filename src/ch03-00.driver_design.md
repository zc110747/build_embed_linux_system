...
在驱动开发中，设备抽象成设备树，厂商和开发者共同维护；总线由linux内核以及部分厂商的私有代码组成(主要为系统内部模块和platform总线代码)；驱动则最为复杂，芯片厂商，器件厂商、BSP维护人员以及产品开发者都可能参与实现。
...
因为不同芯片，内核的差异性，内核驱动接口存在差异，所以参考一套教程就想支持所有平台并不实际，需要自行解决相应软硬件问题。
...
基于接口实现基础的加载和移除的接口，在添加与开发者，驱动的资源信息，即构成驱动模块最基础的框架。

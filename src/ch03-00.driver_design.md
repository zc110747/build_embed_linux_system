...
对于嵌入式Linux项目中驱动的实现，可能只需要打开menuconfig中的配置选项，设备树中的某些设备节点的添加和修改，当然复杂一点当然也会有驱动代码的开发，在使用各大厂商的SDK包时，也只是告知该如何书写才能实现目的。在驱动的开发中，一定被告知过这些知识，在i2c下增加带地址的节点，就可以增加器件；修改屏幕的某个参数，可以适配时钟分辨率；修改cpu节点的某个参数可以改变系统的时钟频率；在menuconfig中，如果需要支持某个设备，去相应目录打开即可。可是如果换一款芯片，虽然大致还是这个思路，但是设备树里的修改项目，以及menuconfig的操作路径会不同，这里似乎并没有那么通用

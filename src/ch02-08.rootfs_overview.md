...
BusyBox，Buildroot，Yocto和Openwrt这些文件系统的构建都涉及到环境构建和编译，这不仅需要很长时间，而且文件系统的编译也十分依赖文件的支持，需要耗费大量的时间经历。参考文件系统是文件和目录的集合，那么有没有将需要的二进制执行文件，脚本，配置文件压缩，用户只需要下载解压，执行安装既可以生成，不需要编译就可以构建系统。

Debian文件系统就是基于此方法构建的(Ubuntu属于Debian的进一步整合，所以实现类似)。当然还有其它的文件系统，如安卓等，虽然构建方式不同，但也没有摆脱这几种方式；理解了这些，就可以更清晰的构建文件系统。

## startup

内核在启动的最后阶段，会查找并执行init程序，之后会挂载文件系统，通过脚本执行必要的初始化，进入shell命令行，这时才是我们常见的命令界面，可以通过串口进行访问和显示。对于嵌入式Linux平台，主流文件系统分为两种。

1. SysVinit管理的系统，这里以基于BusyBox，添加启动脚本最终构建文件系统为例，如BusyBox，Yocto，OpenWrt，Buildroot(两种都支持)等
2. Systemd管理的系统，这一类主要是Debian，Ubuntu这类文件系统，单元为基本管理对象，每个单元代表系统中的一个资源或服务进行管理

### sysvinit

Sysvinit，即System V风格的init系统，是早期Linux发行版中使用的默认init系统。init进程是Linux系统中的第一个用户级进程，其进程号始终为1，负责启动其他用户级进程或服务，系统。这里以BuildRoot的启动流程来说明系统的执行流程。

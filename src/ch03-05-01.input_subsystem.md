...
了解上如何注册input设备后，下面就要讲解如何将input事件提交到input子系统中并通过应用层访问，对于input事件的完整生命周期如下所示。

1. 硬件发生变化，产生相应中断。
2. 中断处理函数被调用，获取I/O状态，并产生input事件。
3. 其中input_report会通过input core检查当前能力，确定是否提交事件，需要则缓存事件。
4. 执行input_sync事件，表示一帧结束，此时将缓存通知给input handler，发送事件到应用层。
5. 应用层收到事件后，根据事件类型，进行相应的处理。

其执行流程如下所示。

```shell
// input事件提交流程
Driver --> Input Device --> Input Core --> Input Handler(evdev) --> /dev/input/eventX -> Application
```
...
input_set_capability则可以同时设置事件和对应的事件位，不过input_set_capability不支持EV_SYN，EV_REP，EV_FF_STATUS的设置，如果希望置位，只能使用__set_bit接口，如__set_bit(EV_REP, input_device->evbit)进行设置。对于触摸屏这类地址的器件，则需要使用input_set_abs_params设置能力，这也是对于__set_bit的封装。

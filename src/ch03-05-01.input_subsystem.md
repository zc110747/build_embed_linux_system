...
在嵌入式Linux系统中，作为设备的眼睛，输入设备数量繁多。

常见的输入设备有输入按键、按键矩阵、扩展I/O、触摸屏、鼠标、键盘等。这些设备接口总类多样，如gpio、i2c、usb等。对于输入设备，其特征差异也十分巨大，各不相同；例如普通按键输入为0/1电平信号、按键矩阵是一组电平信号、触摸屏和鼠标输入是位置信号、键盘输入是对应键位的按键码等，这就需要根据实际硬件类型进行分析。
...
完整的input-key驱动代码可参考: [key-input驱动代码](./file/ch03-05/key_input/kernel_input.c)

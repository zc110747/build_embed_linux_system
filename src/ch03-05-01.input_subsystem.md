...
在嵌入式Linux系统中，作为设备的眼睛，输入设备数量繁多。常见的输入设备有输入按键、按键矩阵、扩展I/O、触摸屏、鼠标、键盘等。这些设备接口总类多样，如gpio、i2c、usb等。

对于输入设备，其特征差异也十分巨大，各不相同。例如普通按键输入为0/1电平信号、按键矩阵是一组电平信号、触摸屏和鼠标输入是位置信号、键盘输入是对应键位的按键码等，这就需要根据实际硬件类型进行分析。

内核为了统一管理这些设备，实现input子系统。在应用层就可以通过访问/dev/input/eventX来访问设备，同时按照特定的格式解析，即可获取触发的硬件信息。

input子系统提供了一套驱动告知应用触发事件的机制，常见的input子系统设备按照输入数据类型的不同可分成如下两类。
...
![image](./image/ch03-05-01-01.png)

1. 硬件输入设备: 作为事件的发起端，带有不同功能的输入设备；如按键、鼠标、键盘、TouchScreen等，作为输入的发起端，是带有具体接口的硬件。
2. 内核空间: 用于将设备加载到系统中，实现具体功能的驱动。主要包含外部设备的配置或枚举和input设备注册到内核中的流程，用于应用层访问。
...
// 设置input设备支持的能力
// @param dev input设备结构体
// @param type 类型 -- 更新evbit
// @param code code码 -- 更新keybit
...
// @param dev input设备结构体
// @param tool_type 触控节点的类型
// @param active true/false表示槽点是否在工作
// @return true/false表示槽点是否在工作
bool input_mt_report_slot_state(struct input_dev *dev, unsigned int tool_type, bool active)

// 作为touchscreen的专用接口
// 解析input设备，获取touchscreen的专有属性
// @param input input设备结构体
// @param multitouch 是否支持多点触控
// @param prop 触摸屏属性结构体
...
#define EV_REP          0x14  // 支持重复触发事件，上层支持连续按键事件
...
驱动中通过`input_set_capability、___set_bit`等方式配置input设备能力的接口。

input设备设置capability后，会在report时检查相应标志位，来决定是否提交，也就是**未正确设置能力标志，会导致应用层无法收到消息。**，为了满足相应功能，需要进行能力的置位，然后将input子系统注册到内核中。

在应用中，input事件从硬件产生到应用层接收，大致经历`硬件中断 → 驱动上报 → Input Core处理 → Input Handler分发 → evdev设备节点 → 应用层读取`六个阶段。其中，input_report_xxx()用于上报具体事件，而input_sync()用于标识一帧事件结束，并触发事件向应用层传递。
...
// 置位KEY事件中的指定key线号
__set_bit(KEY_ENTER, input_device->keybit);
```
...
上述方法都可以实现对input设备能力的配置，不过使用evbit直接设置的方式容易遗漏`BIT_MASK`和`BIT_WORD`，实现并不直观，一般不使用。

input_set_capability则可以同时设置事件和对应的事件位，不过input_set_capability不支持EV_SYN，EV_REP，EV_FF_STATUS的设置，如果希望置位，只能使用__set_bit接口，如__set_bit(EV_REP, input_device->evbit)进行设置。

对于触摸屏这类地址的器件，则需要使用input_set_abs_params设置能力，其内部实现也是对于__set_bit的封装。

### input-key事件提交

上面实现了如何配置触发中断，以及注册input设备，下面一步就是定义事件回调函数，用于上报事件信息。

关于按键触发的事件上报，具体实现如下所示。
...
void key_timer_func(struct timer_list *arg)
{
    ...
    // 读取按键状态，使用gpiod_get_value_cansleep接口，允许睡眠
...
![image](./image/ch03-05-01-02.png)

至此，可以看到判断key作为input设备的驱动实现完成。

完整的input-key驱动代码如下。

- key-input驱动代码: `file/ch03-05-01/key_input/kernel_input.c`

### input-key应用访问

在input接口注册成功后，其应用层同样使用文件接口进行处理。

关于应用层的输入设备访问，其代码如下所示。
...
// 按键按下 | 00 01 EV_KEY | 00 1c EV_ENTER | 00 00 00 01 |
[   46.502569] kernel-key usr_key: key timer interrupt:0!
00000000  70 ee 39 6a 56 85 08 00  01 00 1c 00 01 00 00 00  |p.9jV...........| 
00000010  70 ee 39 6a 56 85 08 00  00 00 00 00 00 00 00 00  |p.9jV...........|
// 按键抬起 | 00 01 EV_KEY | 00 1c EV_ENTER | 00 00 00 00 |
[   56.097502] kernel-key usr_key: key timer interrupt:1!
00000020  7a ee 39 6a 05 5b 02 00  01 00 1c 00 00 00 00 00  |z.9j.[..........|
00000030  7a ee 39 6a 05 5b 02 00  00 00 00 00 00 00 00 00  |z.9j.[..........|

// evtest /dev/input/event0 
Input driver version is 1.0.1
Input device ID: bus 0x0 vendor 0x0 product 0x0 version 0x0
Input device name: "usr_key"
Supported events:
  Event type 0 (EV_SYN)
  Event type 1 (EV_KEY)
...
[  245.947138] kernel-key usr_key: key timer interrupt:1!
Event: time 1782181688.004014, type 1 (EV_KEY), code 28 (KEY_ENTER), value 0
Event: time 1782181688.004014, -------------- SYN_REPORT ------------
[  251.897206] kernel-key usr_key: key timer interrupt:0!
Event: time 1782181693.954087, type 1 (EV_KEY), code 28 (KEY_ENTER), value 1
Event: time 1782181693.954087, -------------- SYN_REPORT ------------
```

对于按键控制代码如下所示。

- input-key应用实现代码：`file/ch03-05/key_input/test/key_input_test.c`
...
![image](./image/ch03-05-01-04.png)

可通过`hexdump -C /dev/input/event1`命令进行测试。

![image](./image/ch03-05-01-05.png)

...
- 功能：通过gpio标签申请gpio管理资源(被管理资源的gpio后续才能控制和配置)
- 目录: drivers/gpio/gpiolib-devres.c
...
- 功能：释放设备中的pinctrl引脚管理句柄(用于显示释放，不使用则在驱动移除时释放)
- 目录: drivers/pinctrl/core.c
...
//参数
p: 申请的管理引脚资源的句柄
name: pinctrl-names中的名称，对应相应的pinctrl-x值

...
PWM, 又称脉冲宽度调制模块，是一种重要的电子控制组件，其工作原理和应用十分广泛。PWM模块通过调整PWM信号的占空比来改变电路中的电流或电压，从而实现对设备的控制。常见的支持PWM输出控制的设备有LED，屏幕背光调节，音频输出，电机控制(直流电机)以及电源管理等。对于PWM来说，需要关注的的配置项有周期，占空比以及极性，详细说明如下。
...
当实现PWM驱动时，就需要在系统中配置PWM的接口，以及驱动中实现对PWM的寄存器操作，后续分别进行说明。PWM属于芯片内部的模块，不属于具体的总线，由内部platform总线统一管理。另外PWM一般不单独作为独立硬件，而是配合控制其它模块的组件，如背光，温控风扇等。

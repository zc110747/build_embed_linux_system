...
PWM, 又称脉冲宽度调制模块，是一种重要的电子控制组件，其工作原理和应用十分广泛。PWM模块通过调整占空比来改变电路中的电流或电压，从而实现对设备的控制。

常见的支持PWM输出控制的设备有LED、屏幕背光调节、风扇转速管理、电机控制(直流电机)以及电源管理等。
...
1. 周期(period): PWM周期是PWM信号中两个有效电平开始的时间间隔，它包含了高电平和低电平两个时间段，是PWM信号频率的倒数。
2. 占空比(duty_cycle): PWM占空比是指脉冲波中有效电平时间(占空时间)与PWM周期的比率，它是PWM信号的一个重要参数。
3. 极性(polarity): PWM极性是指PWM信号的有效电平状态，它决定了PWM信号是高电平有效还是低电平有效。

当实现PWM驱动时，就需要在系统中配置相关硬件，以及驱动中实现对寄存器操作，后续分别进行说明。PWM属于芯片内部的模块，由内部platform总线统一管理。

PWM的框架如下所示。

![image](./image/ch03-12-01.png)

可以看的，作为SOC内部模块，由platform总线管理。PWM子系统一方面配合设备树来管理硬件寄存器，控制硬件输出配置，另一方面作为生产者，可以被其它Consumer或者应用层进行访问控制。

在实践中，PWM一般不单独作为独立硬件，而是配合Consumer成为控制其它模块的组件，如背光、温控风扇、调压模块等，组合实现更复杂的功能，这个特性在学习的过程中需要注意。

## 操作结构和接口

向内核添加PWM驱动的相关接口比较简单，具体如下所示。
...
设备在系统中由PWM子系统通过结构体`struct pwm_chip`管理，这也是框架中最重要的结构，详细说明如下所示。
...
struct pwm_ops {
    int (*request)(struct pwm_chip *chip, struct pwm_device *pwm);      // 请求资源执行的钩子函数
    void (*free)(struct pwm_chip *chip, struct pwm_device *pwm);        // 释放资源执行的钩子函数
    int (*capture)(struct pwm_chip *chip, struct pwm_device *pwm,       
                struct pwm_capture *result, unsigned long timeout);     // 捕获并报告信号
    int (*apply)(struct pwm_chip *chip, struct pwm_device *pwm,
                const struct pwm_state *state);                         // 用于应用新的配置，支持应用层修改需要实现
    int (*get_state)(struct pwm_chip *chip, struct pwm_device *pwm,
                struct pwm_state *state);                               // 获取当前的配置，支持应用层获取状态需要实现
    struct module *owner;                                               // 所属的驱动模块，一般为THIS_MODULE
};
```

## 驱动实现(内部PWM模块)

这里开始讲解如何实现驱动实现的流程。PWM0作为内部模块，也由platform总线进行管理，其实现流程如下所示。

1. 确定硬件，实现对应的设备树，用于驱动匹配和硬件资源获取。
2. 实现驱动驱动注册的接口，匹配设备树的基本框架。
3. 关联硬件，实现访问寄存器的接口，并进行功能的配置。
4. 注册设备到内核，用于应用层访问文件。
...
这一步实现pwmchip如何注册到内核。其中主要部分就是实现`apply和get_state`对应接口，分别用于设置PWM值和获取PWM状态，在配合上面的硬件访问接口进行设置；关联和注册pwmchip设备。
...
至此，完整的的驱动实现完毕；通过将设备注册到内核，就可以被应用层和其它consumer设备访问。

关于PWM模块的的详细代码如下所示。

- PWM驱动代码：`file/ch03-12/pwm_provider/kernel_pwm.c`

## 应用层访问方法

在前面说明过，驱动执行后会在`/sys/class/pwm`目录下增加pwmchipX的目录，用于应用层访问。

根据驱动加载的顺序，本例中虽然使用PWM7，但是因为驱动第二个加载，因此对应pwmchip1。
...
- pwm应用代码实现：`file/ch03-12/pwm_provider/test/pwm_test.c`
...
关于Consumer的代码，详细如下所示。

- Consumer驱动实现：`./file/ch03-12/pwm_consumer/kernel_pwm_consumer.c`

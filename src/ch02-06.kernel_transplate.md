...
Linux内核是Linux操作系统的核心，它负责管理系统的硬件资源，如CPU、内存、磁盘等，同时为用户空间的应用程序提供接口；Linux内核的移植的过程，就是将Linux适配设备硬件，提供接口支持后续应用的功能。

如果只是简单的运行，Linux Kernel的移植十分简单，只需要修改设备树文件和配置文件，就可以完成移植；不过构建完整适配硬件的Linux Kernel，则远比U-Boot复杂的多。对于U-Boot来说，只要实现必要的串口，网络，指示灯等功能，支持打印调试信息以及跳转内核，从应用场景来说基本就合格了。对于内核，则需要适配硬件上的各种外设功能，并确保都工作正常，才能保证系统的稳定和安全，这就需要进行全面的移植和适配。

之前讲述构建运行平台时，说到内核系统代码的三个来源。

1. Linux内核主干代码
2. 芯片原厂适配的Linux内核(有源码方式提供，也有SDK方式提供)
3. 方案商提供的适配自己硬件的Linux内核(一般和U-Boot，Rootfs打包成SDK提供)

Linux内核主干代码是代码的基础，由Linus领导的Linux开源社区维护；芯片原厂基于特定分支的Linux内核，进行芯片硬件的适配，这里以ARM厂商居多，适配芯片的内核，时钟系统，中断系统，GPIO，串口，I2C，SPI等内部模块，以源码的方式提供给开发者使用，当然作为Linux开源社区的一员，同时也将这些代码提交到内核主干中，作为主干内核代码的一部分，厉害的芯片厂商公司，往往也是Linux开源社区提供代码的主力部分。方案商提供的Linux内核，一般是基于芯片原厂提供的内核，进行定制化的适配，包括设备树的修改，驱动的修改，配置文件的修改，以及部分bug的修复，和U-Boot，Rootfs一起打包，SDK的方式提供给开发者使用。本节目录如下所示。
...
对于主干代码，我们已实现最基础的Linux Kernel打印输出功能为例进行测试。因为需要适配的芯片I.MX6ULL，选择适配NXP官方的版本，对应的配置文件和设备树如下所示。
...
上述内核代码在我们开发板是可以直接使用的，不过会报大量的设备驱动错误，这是因为我们的开发板上没有这些硬件设备，所以需要进行适配。直接从主干代码进行移植，工作量是十分大的，而且也不便于维护。所以我们需要选择一种更加简单的方式，就是使用芯片原厂提供的Linux内核，进行适配。
...
修改后的文件详细见[设备树修改后的imx6ul-14x14-rmk.dtsi文件](./file/ch02-06/imx6ul-14x14-rmk.dtsi)

在emmc和nand对应的设备树文件，则增加相应的硬件信息，这部分在[ch03.驱动模块开发](./ch03-00.driver_design.md)中进行详细的添加说明。这里举个例子，例如硬件上i2c1下挂载ap3216c，修改适配网卡，添加和修改设备树节点如下。
...
使用的zImage、imx6ull-14x14-emmc-4.3-800x480-c.dtb和imx6ull-14x14-nand-4.3-800x480-c.dtb即为可以使用的内核移植代码。
...
至此，关于Linux内核的平台移植核适配完毕。可以看到随着统一设备模型的支持，通过设备树的修改基本可以完成大部分的移植工作。另外配置选项的支持，也可以更方便的定制需要的内核，完整的工作目前十分简化了。对于内核本身支持的硬件，只要在设备树中添加，在通过配置项修改，就可以快速支持；对于内核不支持的硬件，就需要通过驱动实现，这部分就可以参考章节03中各模块的功能。对驱动的掌握越深，对于内核的移植就越简单，深入学习驱动相关的知识，也是掌握芯片bringup的必经之路，一起共勉。

...
i2c是嵌入式Linux中最常用通讯接口之一，常见的温湿度、距离传感器，EPPROM存储等器件都支持通过i2c接口进行通信。i2c支持一主多从的硬件结构，通过设备地址区分，只有满足设备地址的器件才会应答，这样就实现总线上同时只有一对一的芯片通讯，也就可以同时挂载多个不同外部器件。当然这也要求**总线上不能有两个相同地址的从设备**，否则会导致数据状态异常(总线上相同地址的设备会同时应答，这样总线电平就会互相干扰，而无法确定)。当然如果设计需要挂载多个相同地址的器件，也可以使用TCA9546A这类多路开关进行i2c器件挂载。

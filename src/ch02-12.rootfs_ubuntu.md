...
注意: 此时网址都需要以http而非https版本访问，因为系统目前未安装ca-certificates，无法访问https版本。
...
# 进行系统更新支持
unminimize
apt-get install libterm-readkey-perl ca-certificates --reinstall -y

# 此时etc/apt/sources.list可以修改为https版本
apt-get update

# 安装ubuntu的必要软件
apt-get install sudo vim systemd kmod -y
apt-get install apt-utils net-tools isc-dhcp-client iputils-ping-y

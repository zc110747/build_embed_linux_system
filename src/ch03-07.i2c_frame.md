...
I2C是嵌入式系统中最常用通讯接口之一，常见的温湿度、距离传感器、EEPROM、RTC时钟器件都支持通过I2C接口进行通信。

在一主多从的硬件结构中，通过设备地址进行区分，只有地址匹配的从设备才会对主设备的请求进行应答；这样就实现总线上同时只有一对一的芯片通讯要求，也就可以同时挂载多个不同外部器件。这种设计也要求**总线上不能有两个相同地址的从设备**，否则会导致数据状态异常(总线上相同地址的设备会同时应答，导致主机无法区分数据来源，返回内容不一致时会产生总线冲突，甚至损坏器件)。如果设计需要挂载多个相同地址的器件，可以使用TCA9546A这类多路开关来实现I2C器件的挂载。

I2C的器件默认为开漏模式，需要通过外部上拉电阻实现高电平输出，更多适用与板级器件或芯片间的通讯。对于长距离通讯，需要外部电路去增强总线(如两端可以增加PCA9615这类单电源电平转换芯片增加可靠性)，从而避免误码和错帧问题，这些在硬件设计时需要注意。
...
可以看到实现完整的I2C功能需要内部外设和外部器件功能实现。

对于SOC内部的I2C总线，需要实现以下功能。

- 配置模块对应的I/O引脚，复用成I2C模式
- 总线的时钟配置
- 总线工作模式配置
- 中断配置、基于总线的数据读取接口
...
当然这是从硬件角度去理解驱动一个标准I2C器件需要实现的功能，那么在嵌入式Linux系统中，也是将硬件抽象成如下的实现。

![image](./image/ch03-07-02.png)

如上图所示，这里的i2c adpater就是内部外设总线控制器。对于I2C器件来说，外部器件的正常工作，需要I2C总线和器件的实现。I2C总线通过初始化内部模块实现，设备则是外部器件节点, 而驱动则主要是由用户开发，包含具体器件内部寄存器功能的应用实现。

基于统一设备模型，完整的I2C总线和外设框架如下所示。
...
一个完整的I2C驱动，包含I2C设备树节点、模块驱动(同时实现i2c总线)、器件节点和器件驱动。不过对于大部分SOC来说，本身厂商在发布SDK时，一定会支持I2C模块驱动，不需要开发，因此本节主要以器件驱动进行说明。

## bus_adpater

前面提到，由设备树通过i2c adpater实现内部I2C模块驱动.

以I.MX6ULL的I2C1为例，节点信息如下。
...
这就是官方实现的i2c adpater驱动实现。参考代码中框架可以发现i2c adpater由platform_driver_register进行注册，也就是说对于I2C模块，也被抽象成挂载在platform总线的设备。

关于I2C注册到内核中的代码，具体如下所示。
...
可以看到设备树非节点的配置向，会在此处进行处理，实现I2C模块的功能配置，也就是i2c-bus的功能实现。这部分就是内部模块的驱动，也是统一设备模型中总线的实现部分。另外，总线初始化完成后，会0在/sys/bus/下创建对应总线，并在/sys/bus/i2c/devices/中生成相应的器件节点，这样就可以在驱动中使用I2C总线接口来进行器件驱动的加载。
...
可以看到，对于IMX6ULL来说，支持两条总线I2C0和I2C1。对于每条总线，又根据在设备树节点下的器件创建不同的设备节点，这些节点可通过reg属性进行区分和查看。

另外使用i2cdetect查看当前i2c总线上的设备。
...
本节以ap3216c为例，说明如何实现i2c外部器件的驱动；主要包含如下内容。
...
主要实现字符设备的创建，提供应用层访问的open、read、writ、close接口，这一部分可以参考[字符设备驱动实现](./ch03-03.char_device.md)中关于设备号申请、字符设备创建、类创建等流程。

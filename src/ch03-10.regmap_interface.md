...
了解过i2c, spi这类接口，可以看到都是通过总线连接，且以寄存器的形式去访问地址，那么能否抽象成统一的接口，以读地址的方式去读写相应的数据？regmap框架正是在这一思路下实现，用于屏蔽访问接口差异，以寄存器的方式统一进行处理，regmap框架结构如下所示。
...
可以看到，regmap只是取代了驱动中i2c，spi接口读写的部分，其它设备树，设备注册，访问并没有区别，这是运用regmap最重要的部分。当你使用regmap框架来开发驱动时，它本身是什么类型的设备，其处理并没有变化。例如它是i2c接口的传感器，那么你仍然要实现设备树，创建字符设备，实现操作接口，这部分还是一致的。只是操作i2c，spi，寄存器读写的接口，需要替换成regmap_read/regmap_write这类接口，另外还需要调用相应的init接口，初始化后才能使用这些接口。一定要有i2c，spi，mmio接口上设备原有访问的概念，才能实现基于regmap通用访问办法。
...
struct regmap_config {
    ...
    int reg_stride;         // 寄存器地址步长，用于多字节写入递增的个数
    ...
    unsigned long write_flag_mask;                      //**在写操作时，在寄存器的顶部字节设置掩码(需要更加器件确定**
...
这里解释下，前面说到regmap是统一内部模块，spi，i2c，mdio等接口，访问相应硬件的框架。而作为这类器件，除了接口，对于硬件上访问的差异是客观存在的，这里列出一部分。

1. 器件的单次读写长度不一致，例如传感器有1字节的，如扩展芯片pcf8563, 4字节的，如内部模块寄存器，还有一些64位芯片，内部模块寄存器需要8字节读取
2. 器件是否支持单次多读
3. 对于器件内部的寄存器也有可读，可写，只读，只写的差异
4. 一些特殊器件，地址位的某些位定义对寄存器的访问方式，读或者写

这些差异最终要反映到通讯的实际电平上，自然需要配置项去定义，配置项虽然多，但是对于开发者来说，常用的配置如下所示。

- reg_bits: 定义寄存器地址的bits数，1字节为8，2字节为16，依次类推，必选
- val_bits: 定义单次读取寄存器内部值得bits数，必选
- reg_base: 访问寄存器时，定义得基地址，实际地址为(reg_base + reg_addr), 以实际地址0x120为例，如果定义基地址为0x100, 这样寄存器使用0x20既可以访问，这对于内部模块寄存器访问可以简化操作，大部分使用默认0即可。
...
可以看到，regmap是对现有的i2c，spi，寄存器访问接口的封装。通过统一处理，简化了驱动实现的难度。不过对于开发人员，还是需要理解基础接口的实现方式，才能更方便的使用regmap实现具体功能。

本节相关代码如下。

- [ap3216完整驱动代码](./file/ch03-10.remap_subsystem/kernel_regmap_i2c.c)
- [pcf8563完整驱动代码](./file/ch03-10.remap_subsystem/kernel_regmap_rtc.c)
- [icm20608完整驱动代码](./file/ch03-10.remap_subsystem/kernel_regmap_spi.c)

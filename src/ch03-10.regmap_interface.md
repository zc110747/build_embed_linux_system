...
了解过I2C、SPI这类接口，可以看到都是通过总线连接，且以寄存器的形式去访问器件中的数据。那么这些访问能否抽象成统一的接口，以读地址的方式去读写相应的数据？regmap框架正是在这一思路下实现，用于屏蔽访问接口差异，以寄存器的方式统一进行处理。

本节regmap框架的使用和说明，具体目录如下所示。
...
对于regmap系统来说，其框架结构如下所示。

![image](./image/ch03-10-01.png)

🚀 **底层物理总线**：regmap框架对不同的物理总线进行了封装。支持的主要物理总线包括I2C、SPI、MMIO（内存映射I/O）等。这一层负责提供与具体物理总线相关的操作函数  
🚀 **regmap核心层**：regmap框架的核心功能，包括寄存器的访问、缓存管理等。开发人员不需要关心这一层的具体实现细节  
🚀 **regmap API抽象层**：向驱动开发人员提供了统一的API接口，用于操作具体的芯片设备。驱动开发时使用这些API接口来读写寄存器、更新寄存器的位等  
...
可以看到，regmap框架只是取代了驱动中I2C、SPI接口读写的部分；对于设备树、设备注册、应用访问并没有区别。使用regmap框架来开发驱动时，它本身是什么类型的设备，其处理流程并没有变化。

对于I2C接口的传感器，仍然要实现设备树、创建字符设备、实现操作接口，这部分是一致的。只是操作接口访问寄存器读写时，将原先的硬件总线接口替换成regmap_xxx接口；

和总线接口配置类似，regmap也需要调用相应的init初始化用于适配器件特性。基于前述知识，可以把regmap看作对总线接口更高层次的封装；使用regmap框架时，要对原有硬件总线接口有一个清晰的认知，才能更好的使用这也是理解regmap框架的基础。
...
```c
// 为SPI设备创建并初始化一个由devm管理的regmap对象
// @dev: SPI设备对象
// @config: 配置参数，定义SPI设备的访问格式
// @lock_key: 用于Linux内核锁调试机制(lockdep)，驱动无需关注
// @lock_name: 内部锁名称,用于lockdep调试显示, 驱动无需关注
// @返回：返回struct regmap指针，失败返回ERR_PTR(errno)
...
// 为I2C设备创建并初始化一个由devm管理的regmap对象
// @dev: I2C设备对象
// @config: 配置参数，定义I2C设备的访问格式
// @lock_key: 用于Linux内核锁调试机制(lockdep)，驱动无需关注
// @lock_name: 内部锁名称,用于lockdep调试显示, 驱动无需关注
// @返回：返回struct regmap指针，失败返回ERR_PTR(errno)
...
// 通过regmap框架向设备指定寄存器带掩码的写入值。
// @map: 管理regmap的格式
// @reg: 寄存器的首地址
// @mask: 设置掩码，设置为1的位才会被修改
// @val: 需要更新的值
// @返回: 成功返回0，失败返回负的错误码
int regmap_update_bits(struct regmap *map, unsigned int reg, unsigned int mask, unsigned int val)

// 通过regmap框架向设备指定的一组寄存器写入值。
// @map: regmap对象指针
// @reg: 要写入的寄存器的首地址
// @val: 寄存器写入数据的指针首地址
// @val_count: 需要写入的值的数量
...
可以看到，对于接口中，最重要的结构就是用于初始化的`struct regmap_config`和保存信息的`struct regmap`属性；其中struct regmap大部分都是config中的信息，因此这里以regmap_config进行解释。

regmap_config的详细说明如下所示。
...
理解了这些，似乎就可以实现regmap的访问了。

然而使用regmap框架进行驱动开发的现实面临两大问题。

❓ *为什么这么多配置选项，在开发中主要配置有哪些*  
❓ *对于具体的I2C，SPI器件或者内部模块，该如何定义初始化配置，实现功能*  

这里解释下，前面说到regmap框架用于统一管理内部寄存器和硬件接口，访问相应器件。对于这类器件，除了接口，对于硬件上访问的差异是客观存在的，这里列出一部分。
...
至于其它参数，虽然也同样各有用途，不过只在特定器件或者希望附加功能时配置，使用默认的值也基本能实现功能，可以先不关注，遇到了在总结即可。

下面将以实际应用来分析regmap的实现，因为这些器件的注册和使用在其它章节都有详细说明，这里不在描述如何加载驱动，注册设备文件的实现，专注于封装操作硬件的接口。

## 驱动接口实现

regmap是对器件操作的封装，那么我们在使用时，最重要的就是理解器件说明中的关键信息。
...
这些都是在芯片手册定义的，这里以之前讲解过的ap3216、icm20608以及RTC芯片PCF8563进行说明。

### regmap驱动实现(基于ap3216器件)
...
📌 **devm_regmap_init_i2c**：用于初始化  
📌 **regmap_read**：用于读取器件内部寄存器  
📌 **regmap_write**：用于写入器件内部寄存器  

详细的驱动代码实现如下所示。

📀 ap3216驱动实现：`file/ch03-10/regmap_ap3216c/kernel_regmap_i2c.c`

### regmap驱动实现(基于pcf8563器件)
...
📌 **devm_regmap_init_i2c**：用于初始化
📌 **regmap_bulk_read**：用于多字节读取器件内部寄存器
📌 **regmap_bulk_write**：用于多字节写入器件内部寄存器

详细的实现如下所示。

- PCF8563驱动代码：`file/ch03-10/regmap_pcf8563/kernel_regmap_rtc.c`

### regmap驱动实现(基于icm20608器件)

关于SPI的配置说明来自于icm20608的手册，具体如下所示。
...
📀 I2C20608驱动代码：`file/ch03-10/regmap_icm202608/kernel_regmap_spi.c`

## 总结说明

可以看到，regmap是对现有的总线访问接口的封装；通过统一处理，简化了驱动实现的难度。

从regmap的操作流程来看，都是先写寄存器地址，再写操作数据，这样就只能用于带寄存器的外部器件。对于部分I2C或SPI器件，不存在寄存器，只支持直接操作数据，这时候就不符合regmap的操作方式，只能使用基础的总线框架进行管理。为了应对开发中的复杂情况，开发人员仍然需要理解基础接口的实现方式，才能更方便的使用regmap实现具体功能。

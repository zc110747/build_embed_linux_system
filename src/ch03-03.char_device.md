...
从上面可以看出，日常开发中大部分的设备都属于字符设备。网络设备和块设备虽然也多，但一般都使用系统自带的驱动，很少单独开发。目前主要的网络设备包含以太网、CAN、WIFI、蓝牙等；块设备则通常则是SD卡、Nand、EMMC、固态和机械硬盘等，这些设备类型将在后续进行说明。

本节主要讲解字符设备驱动，将从以下几个方面讲解说明。

- [字符设备驱动说明](#字符设备驱动说明)
  - [字符设备处理接口](#device_interface)
  - [硬件访问接口](#hardware_interface)
  - [创建字符设备](#char_device_create)
    - [字符设备接口实现](#char_device)
    - [杂项设备接口应用](#misc_device)
...
1. 如何在系统中添加字符设备，以及字符设备驱动中实现的API接口，包括杂项设备。
2. gpio设备树、驱动中解析和操作方法。
3. 使用文件管理字符设备的方法。
4. 应用层访问字符设备的接口。
...
// 说明: 创建杂项设备
// @misc: 杂项设备结构体指针，用于关联字符设备结构体和字符设备操作接口
// 返回：0表示申请成功，其它返回错误码
...
- 实现硬件操作接口(xxxx_set), 控制引脚输出高/低电平。
- 应用中通过调用操作接口的方式，控制实际硬件输出。

对于嵌入式Linux系统，其实现逻辑也类似。也可以直接配置寄存器实现功能和引脚控制。
...
对于单片机来说，往往每家都有独立的驱动和SDK包，统一的需求并不大，这个问题并不严重。对于Linux系统来说，需要兼容不同指令集、厂商、芯片的驱动需求，实现统一的管理。统一设备模型被设计出来，其中设备树用于描述硬件差异，驱动则解析设备树实现具体功能。

注意: **统一设备模型通过设备树来区分板级信息差异，不过这并没有彻底解决问题。举个例子，全志芯片的驱动大概率不能直接被NXP、瑞芯微的芯片使用；不过这仍然降低了驱动的碎片化，对于同一家厂商的驱动，一般都会前后兼容，只需要更新相应的设备树即可，而不需要不同的板级信息就需要更新驱动，从这个角度已经是很大的进步了。**

这里以控制蜂鸣器输出的I/O来展示字符设备驱动的实现，具体步骤实现。
...
static int beep_hardware_init(struct beep_data *chip)
{
    ...
    dev_info(&pdev->dev, "beep_hardware_init is active:%d\n", gpiod_is_active_low(chip->desc));
    return 0;
}
...
ssize_t beep_write(struct file *filp, const char __user *buf, size_t cnt, loff_t *f_pos)
{
    ...
    // 字符'0'转换为0, 兼容应用层echo 0 > /dev/beep写入
    if (data >= '0' && data <= '9') {
        data = data - '0';
    }
...
1. 初始化硬件接口和设备，对于挂载在platform总线的设备如gpio、pwm、adc等，只需要配置内部模块即可。对于外部设备，如i2c、spi等内部模块，除了配置内部外设，还需要配置外部器件。
...
7. llseek：设置文件指针位置的函数(文件读取时修改偏移值)，对应应用层的lseek。
...
对于字符设备的创建方法，归纳起来如下所示。

可以看到，字符设备的创建步骤如下所示。

1. 申请设备号，可自定义设备号或者从系统中获取空的设备号。
2. 初始化字符设备，关联字符设备支持的硬件接口。
3. 将字符设备和设备号关联，并添加到系统中(cat /proc/devices)。
4. 在系统中创建设备类文件和设备(ls /dev/)，关联设备号。

这样在文件系统中访问设备文件时，就可以查找关联的字符设备，从而最终能够操作实际硬件。

关于字符设备的申请和创建流程，详细如下所示。
...
杂项设备(misc)也是字符设备的一种，用于生成设备节点。不过杂项设备是固定主设备号的设备(主设备号固定为10)，通过次设备号来区别不同的设备。

使用杂项设备可以简化字符设备的创建过程, 杂项设备的结构定义如下。
...
- minor: 定义子设备号，需要确保未被占用
- name：设备名称，可以在/dev/目录下查看
...
注意：**杂项设备中的open函数中，flip->private_data中在调用前会保存为miscdevice结构体，使用此数据结构，即可获取到完整的系统设备参数**。
...
sysfs是Linux内核中向用户空间导出对象的虚拟文件系统。它提供了一种机制，用于将内核对象、驱动程序和设备的信息以文件和目录的形式呈现给用户空间。

通过sysfs用户空间程序可以方便地与内核进行交互，获取设备信息、配置设备参数等。sysfs并不实际存储数据在磁盘上，而是在内存中维护数据结构，通过文件和目录的形式将这些数据暴露给用户空间，可以通过"ls /sys"目录查看。

常用sys目录核功能说明如下。

1. `/sys/bus`：包含各种总线类型的目录，如pci、usb、i2c等。每个总线目录下包含该总线上连接的设备和驱动程序的信息。
2. `/sys/class`：按设备类型分类的设备类目录，如block（块设备）、char（字符设备）、net（网络设备）等。每个设备类目录下包含该类型的所有设备。
3. `/sys/dev`：包含设备节点的符号链接，指向/sys/class或/sys/bus下的具体设备目录。
4. `/sys/devices`：包含系统中所有设备的目录树，按照设备的拓扑结构组织。
5. `/sys/module`：包含内核模块的信息，每个模块对应一个目录，目录下包含模块的属性和参数。
6. `/sys/firmware`：包含与固件相关的信息。

sysfs将内核中的各种对象（如设备、驱动程序、总线等）映射为文件和目录，用户可以通过读写这些文件来获取或修改内核对象的属性。在内核中，依赖"struct kobject"结构来显示和管理设备，常见的接口如下。
...
//具有相同功能的宏定义
struct kobj_attribute led_attr = __ATTR(led, 0644, led_show, led_store);
...
不过对于驱动中一般使用更上层的封装接口，`device_create_file`和`device_remove_file`接口，最终也会调用上述sysfs接口，在设备下创建sysfs目录和文件。

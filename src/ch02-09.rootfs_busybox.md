...
对于嵌入式项目开发来说，往往使用Buildroot, Debain, OpenWRT这类系统作为应用执行的平台，很少单独只使用Busybox来构建系统，这是因为Busybox只有系统需要的最基础目录和功能，如果进行开发，需要进行十分多的第三方软件移植，是十分低效的方式。不过从学习的角度来说，BusyBox又是比较适合入门的系统，可以理解构建的系统需要文件和执行流程，具体如下所示。

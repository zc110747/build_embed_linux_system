...
对于linux平台的软件的交叉编译，如何总结其实是很复杂的问题。不同版本的系统、芯片平台，甚至不同版本的编译器，编译时面临的问题可能都不同。除非直接使用虚拟机或者docker进行完整的复制，否则面对的情况都可能不同。

这里列出我编译交叉编译时遇到的问题和解决办法，可以作为解决问题的参考思路。

- 编译器的版本不支持某些编译选项，例如-m32、-m64等。这种一般发生在旧版本gcc编译新的软件或者库，新版本gcc编译旧版本软件或者库。

编译器版本问题一般是软件版本和编译器不匹配，部分特性不支持导致编译错误。这时就不要尝试修改软件中的配置项来适配，这是吃力不讨好的行为，即使编译成功也可能在某些情况下不能正常工作。一般建议编译器选择尽可能新的版本，软件同理选择最后的LTS版本；新版本软件往往会修复更多的安全和使用功能问题。当然也有一些软件可能已经放弃了更新，停留在几年前版本，这种情况可能只支持老版本编译器，遇到这种问题不要去死磕，直接看是否有同类软件的替代品或者自行维护。

- 编译的系统平台不支持某些应用

如果使用的是基于VMware, Virtualbox这类完整的虚拟机，一般不会出现此问题。我是在使用wsl1时遇到过，因为wsl1的特性是微软在windows上模拟了一套Linux的API，因此很多Linux基础特性是缺失的，在日常使用时可能差别不大，但在做Linux系统平台编程和交叉编译时，说不定什么时候就会遇到问题，这里建议使用wsl2或者直接使用虚拟机创建完整Linux环境，避免这类问题。
...
找不到库文件和上述类似，本平台编译时会查找"/lib/"，"/lib32"，"/usr/lib/x86_64-linux-gnu/，"/usr/lib"目录，如-lcrypto查找libcrypto.so，-lssl查找libssl.so，这时可以使用apt-get安装。当然安装成功后并不一定链接成功，可能Linux版本较老，导致即使链接到对应的库，也不一定能编译成功，如果失败还是用源码编译，参考交叉编译的方式，下载依赖库的源码，编译下载后通过配置选项引用到项目中。
...
上面就是交叉编译时遇到的常见问题分类，在实际开发过程中要根据情况，具体问题具体分析。有些软件只需要configure配置下，有些软件直接就没考虑交叉编译的环境支持(mosquitto，nginx等)，就需要去**修改软件的Makefile和configure来满足要求**，记住这些开源库其实也是软件，是可以直接去修改和配置的，如果能够掌握shell，并理解cmake语法，那么这些问题都有相应的解决方法。

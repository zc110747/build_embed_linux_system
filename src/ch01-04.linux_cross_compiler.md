...
Linux平台软件安装是通过软件源提供一套兼容当前系统软件和库，通过apt命令下载安装的过程，不一定是新版本，而是整合成系统包时的版本。而软件编译则不同，可以去各软件官网查询最新的版本，进行编译。其中软件的交叉编译更加复杂，是在不同平台上交叉编译另一平台的软件，这就相当于需要自行解决所有的编译依赖问题。

对于交叉编译，这里建议选择固定的Linux版本(本文在Ubuntu 22.04 LTS版本验证)。对于Linux平台软件的交叉编译，不仅依赖交叉编译工具，还可能依赖gcc、make、python2/python3、perl、scons等工具，如果编译时缺失，就需要根据实际情况去解决支持。另外还有软件依赖某些基础组件如libssl.so，libcrypto.so，libzlib.so等库的支持，Linux工具的交叉编译，往往除本身的编译外，还涉及相关支持库的编译，这就大大增加了难度，对开发者有了更高的要求。
...
- 💻 [can-utils软件交叉编译](#can_utils_cross_compiler)
...
对于linux平台的软件的交叉编译，如何总结其实是很复杂的问题。不同版本的系统、芯片平台，甚至不同版本的编译器，编译时面临的问题可能都不同。除非直接使用虚拟机或docker进行完整的环境复制，否则面对的情况都可能不同。

这里列出我编译交叉编译时遇到的问题和解决办法，可以作为解决问题的参考思路。

- 编译器的版本不支持某些编译选项，例如-m32、-m64等。这种一般发生在编译器不属于库验证过的版本。如旧版本gcc编译新的软件或库，以及新版本gcc编译旧版本软件或库。

编译器版本问题一般是软件版本和编译器不匹配，部分特性不支持导致编译错误。这时就不要尝试修改软件中的配置项来适配，这是吃力不讨好的行为，即使编译成功也可能在某些情况下不能正常工作。一般建议编译器选择尽可能新的版本，软件同理选择最后的LTS版本；新版本软件往往会修复更多的安全和使用功能问题。当然也有一些软件可能已经放弃了更新，停留在几年前版本，这种情况可能只支持老版本编译器，遇到这种问题不要去死磕，直接看是否有同类软件的替代品，在实在没办法且必须的情况下，才去进行软件的迭代适配。

- 编译的系统平台不支持某些应用

对于VMware, Virtualbox这类完整的虚拟机，一般不会出现此问题。我是在使用wsl1时遇到过，因为wsl1的特性是微软在windows上模拟了一套Linux的API，因此很多Linux基础特性是缺失的，在日常使用时可能差别不大，但在做Linux系统平台编程和交叉编译时，说不定什么时候就会遇到问题。这里建议使用wsl2或者直接使用虚拟机创建完整Linux环境，避免这类问题。

- 找不到头文件，如找不到#include <cJSON.h>。

找不到头文件的情况根据编译目的不同，解决难度也不一样。在本平台上编译解决起来比较简单，主要是缺少依赖库。这是因为软件在编译时会查找`/usr/include, /usr/local/include`目录，找不到则报错。这时解决步骤就是使用apt search找到相应的库，然后使用apt-get安装。

对于交叉编译，则不会去访问系统的库，这时就需要下载依赖库的源码，下载、编译安装后，使用"./configure"的配置选项引用到项目中。

- 链接找不到库文件，如找不到ssl库。

找不到库文件和上述类似，本平台编译时会查找`/lib/，/lib32，/usr/lib/x86_64-linux-gnu，/usr/lib`目录；如-lcrypto查找libcrypto.so，-lssl查找libssl.so，这时可以使用apt-get安装。当然安装成功后并不一定链接成功，如Linux版本较老，即使链接到对应的库，也不一定能编译成功，此时只能找到依赖库的源码编译，这时可以参考交叉编译的方式；下载依赖库的源码，编译下载后通过配置选项引用到项目中。

如果不知道需要下载什么库，可通过如下命令查找需要安装的库。
...
上面就是交叉编译时遇到的常见问题分类，在实际开发过程中要根据情况，具体问题具体分析。有些软件只需要configure配置下，有些软件直接就没考虑交叉编译的环境支持(mosquitto，nginx等)，就需要去**修改软件的Makefile和configure来满足要求**。在交叉编译时一定要牢记一点，这些开源库其实也是软件，是可以直接去修改和配置的。如果能够掌握shell，并理解cmake语法，那么这些问题都有相应的解决方法。

当然，时间不紧急又无法解决的情况下，也可以将发生异常的issue完整的提交到开源软件网站(如github)，等待更新修复也是一种办法。

本篇中很多软件都是按照上述思路进行交叉编译的，当然这些软件也只是Linux系统软件的冰山一脚，后续会持续更新。

注意：**交叉编译时，一定要重定义安装目录，否则会替换系统中的库文件，此时会因为平台差异导致无法链接和运行，影响系统功能使用。**.

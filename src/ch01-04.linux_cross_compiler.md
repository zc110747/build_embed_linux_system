...
对于交叉编译，安装目录统一为: ${PROGRAM_INSTALL}, 可在编译前自定义

```shell
#存放安装信息的目录
export PROGRAM_INSTALL="/home/freedom/Desktop/sdk/arm/install"
```
...
./configure --prefix="${PROGRAM_INSTALL}" --host=arm-none-linux-gnueabihf
...
#通过perl完成Makefile的生成
./config --cross-compile-prefix=arm-none-linux-gnueabihf- no-asm --prefix="${PROGRAM_INSTALL}" linux-armv4
make -j4 && make install
```
...
```shell
    ...
    #进入并配置指定ssl和zlib
    cd openssh-9.5p1/
    ./configure --host=arm-none-linux-gnueabihf --with-libs --with-zlib="${PROGRAM_INSTALL}"/ --with-ssl-dir="${PROGRAM_INSTALL}" --disable-etc-default-login
...
export DESTDIR="${PROGRAM_INSTALL}/mosquitto"
...
./configure --prefix="${PROGRAM_INSTALL}" --with-http_ssl_module --with-http_mp4_module --with-http_v2_module --without-http_upstream_zone_module --add-module=/home/program/download/source/nginx-rtmp-module
...
CPP=${CC} -E
LINK=$(CC)
CFLAGS+=-I "${PROGRAM_INSTALL}"/include
LINK+=-L "${PROGRAM_INSTALL}"/lib
#--------------------------------------------------------
...
cd ffmpeg-6.1/ 
./configure --arch=arm --target-os=linux --cross-prefix=arm-none-linux-gnueabihf- --prefix="${PROGRAM_INSTALL}"
...
export CC=arm-none-linux-gnueabihf-gcc
./configure --host=arm-linux --prefix="${PROGRAM_INSTALL}" --enable-shared=yes --enable-static=no --disable-python
...
./configure CC=arm-none-linux-gnueabihf-gcc LDFLAGS="-L"${PROGRAM_INSTALL}"/lib -lasound -latopology" CFLAGS="-I${PROGRAM_INSTALL}/include" --host=arm-linux --prefix="${PROGRAM_INSTALL}" --enable-static --disable-xmlto --disable-alsamixer --with-libiconv-prefix="${PROGRAM_INSTALL}" --with-systemdsystemunitdir="${PROGRAM_INSTALL}"/lib/systemd/system --with-udev-rules-dir="${PROGRAM_INSTALL}"/lib/udev/rules.d --with-asound-state-dir="${PROGRAM_INSTALL}"/var/lib/alsa
...
cd libnl-3.2.25
./configure --host=arm-none-linux-gnueabihf --prefix=${PROGRAM_INSTALL} 
...
./configure CC=arm-none-linux-gnueabihf-gcc CXX=arm-none-linux-gnueabihf-g++  --host=arm-linux --prefix="${PROGRAM_INSTALL}"
...
cd ncurses-6.4
./configure --host=arm-none-linux-gnueabihf --prefix="${PROGRAM_INSTALL}"--without-tests --with-shared  --disable-stripping --without-progs
make && make install
```
...
./configure --host=arm-none-linux-gnueabihf --prefix="${PROGRAM_INSTALL}" CPPFLAGS=-I/home/freedom/sdk/arm/install/include/ncurses LDFLAGS=-L/home/freedom/sdk/arm/install/lib
make && make instal

...
PCRE是一个用C语言编写的库，它实现了与 Perl 5 相同的正则表达式语法和语义。
...
for i in /etc/init.d/S??* ;do
    ...
    if type psplash-write >/dev/null 2>&1; then
        TMPDIR=/run psplash-write "PROGRESS $progress" || true
    fi
done
########################################################
```
...
直接开始下一小节:[Linux快速部署SDK方法](./ch01-05.linux_quick_deploy.md)。

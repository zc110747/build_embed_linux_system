...
块设备驱动时操作系统中用于管理以数据块为单位进行数据读写的设备的软件模块，常见的块设备包含硬盘，固态硬盘（SSD），SD卡等。块设备驱动作为字符设备，网络设备并列的三大驱动之一，也是操作系统中最复杂的一类驱动。硬盘，SD卡等都是数据存储的硬件，以SD卡为例，内部提供了类似emmc的数据存储空间，保存数据。为了方便管理数据，将数据固定的格式划分存储，通过表的方法查询管理资源，这就是分区和文件系统的实现背景。常见的文件系统有fat32，ext4等，其中分区数据数据信息保存在SD卡的固定位置，也就是MBR/GPT分区表。

SD卡作为独立设备，不配合主设备访问数据没有意义，这里就需要物理接口连接。例如SD/MMC接口就可以访问SD卡数据，这部分驱动实现对于SD卡的读写操作，当读取到SD卡信息，此时就会执行对应的块设备驱动，向系统中注册块设备，此时在系统的/dev就会创建对应的块设备文件。此时使用mount就可以正常的访问SD卡的数据。
...
直接开始下一章节说明: [USB驱动管理框架](./ch03-21.usb.md)

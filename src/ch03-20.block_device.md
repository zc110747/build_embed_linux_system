...
块设备驱动时操作系统中用于管理以数据块为单位进行数据读写的设备的软件模块，常见的块设备包含硬盘，固态硬盘（SSD），SD卡等。块设备驱动作为字符设备，网络设备并列的三大驱动之一，也是操作系统中最复杂的一类驱动。硬盘，SD卡等都是数据存储的硬件。

以SD卡为例，内部提供了类似emmc的数据存储空间，保存数据。为了方便管理数据，将数据固定的格式划分存储，通过表的方法查询管理资源，这就是分区和文件系统的实现背景。常见的文件系统有FAT32，EXT-4等，其中分区数据数据信息保存在硬件的固定位置，也就是MBR/GPT分区表。和之前的字符设备不同，块设备只能以块为单位接收输入和返回输出；请求一般有缓冲区，支持随机访问，不过顺序访问往往会有更快的速度。
